...
**本数据库只存关于知识库的元数据，不存知识本身。**

| 允许存的（元数据） | 禁止存的（知识） |
|------------------|----------------|
| 文件路径、标题、字数、修改时间 | 笔记正文、段落、摘要 |
| tag 列表、链接数、目录 | 概念解释、观点、论证 |
| Inbox/任务/项目/会话状态 | 从笔记提取的"知识条目" |
| CSV 镜像字段 | 任何形式的"内容缓存" |

**为什么：** 数据库是工具，不是知识载体。知识属于 `.md` 笔记，归主人所有。Codex 若把"知识"提取进 DB，等于偷主人的内容到自己的领地——这是边界违规。

**审计：** `python mpc/audit_db.py` 可检查所有 TEXT 字段是否超出元数据范畴。
