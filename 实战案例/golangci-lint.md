...
# golangci-lint · 项目深度解析

> Go 静态分析聚合器，集成 50+ linter
> 来源：golangci-lint-main.zip
...
|---|---|
| 项目名 | golangci-lint |
| 一句话定位 | Go 静态分析聚合器，集成 50+ linter |
| 核心问题 | 解决「linter 并行调度 + 结果聚合」领域的核心痛点：Go 静态分析聚合器，集成 50+ linter |
| 目标用户 | Go 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
- `pkg/golinters/clickhouselint/testdata/go.mod`
- `pkg/golinters/exptostd/testdata/go.mod``

**实际代码入口**：`cmd/golangci-lint/main.go`

**核心目录**（文件数最多）：`pkg/commands/internal/migrate/testdata/yaml`, `pkg/result/processors`, `jsonschema`, `pkg/config`, `pkg/printers`, `pkg/golinters/ginkgolinter/testdata`
...
**[点状解析]**：linter 项目的核心架构看点是 **linter 并行调度 + 结果聚合**。
...
**核心架构看点**（linter 并行调度 + 结果聚合）：
- linter 并行调度 + 结果聚合
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 linter 方向**
- 状态：已采纳
- 背景：解决「linter 并行调度 + 结果聚合」领域的核心痛点：Go 静态分析聚合器，集成 50+ linter
- 决策：采用 linter 并行调度 + 结果聚合 作为核心架构思路
- 理由：该方向在 linter 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `pkg/golinters/lll/testdata/lll_max_scan_token_size_cgo.go`
2. `pkg/golinters/lll/testdata/lll_max_scan_token_size.go`
3. `pkg/config/linters_settings.go`
4. `pkg/commands/internal/migrate/migrate_linters_settings.go`
5. `pkg/lint/lintersdb/builder_linter.go`
```

**入口文件**：`cmd/golangci-lint/main.go`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：cmd/golangci-lint/main.go
...
- **linter 并行调度 + 结果聚合**：这是 golangci-lint 的"灵魂"功能，必须精读
- **linter 并行调度 + 结果聚合**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **linter 并行调度 + 结果聚合**（golangci-lint 的核心）
   - 实现思路：该方向在 linter 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：linter 并行调度 + 结果聚合
   - 自己项目：可借鉴到 开源 + 周边服务

2. **linter 并行调度 + 结果聚合**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 golangci-lint 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 cmd/golangci-lint/main.go → 理解启动流程
- D3: 读核心目录 `pkg/commands/internal/migrate/testdata/yaml`, `pkg/result/processors`, `jsonschema`, `pkg/config`, `pkg/printers`, `pkg/golinters/ginkgolinter/testdata` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-golangci-lint（只保留核心）
- D6: 把 linter 并行调度 + 结果聚合 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | golangci-lint 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. linter 并行调度 + 结果聚合：该方向在 linter 领域已被广泛验证，兼顾性能、可维护性与生态
2. linter 并行调度 + 结果聚合：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. cmd/golangci-lint/main.go — 启动流程
2. pkg/golinters/lll/testdata/lll_max_scan_token_size_cgo.go — 核心实现
3. pkg/golinters/lll/testdata/lll_max_scan_token_size.go — 关键算法
4. pkg/config/linters_settings.go — 性能优化
5. pkg/commands/internal/migrate/migrate_linters_settings.go — 边界处理

## 1 个反模式
- linter 常见过度设计

## 1 个可复用模式
- linter 并行调度 + 结果聚合 实现方式

## 我能马上用的 3 件事
1. [ ] 把 linter 并行调度 + 结果聚合 拆成 3 个步骤
2. [ ] 学 linter 并行调度 + 结果聚合 写一个 mini-golangci-lint
...
> golangci-lint 作为 linter 类项目，它的独特看点：

- **linter 并行调度 + 结果聚合** → 该方向在 linter 领域已被广泛验证，兼顾性能、可维护性与生态
- **linter 并行调度 + 结果聚合** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践

**与同类的对比**：
vs 其他 linter 方案：特点突出
...
> 解析 golangci-lint = 计划书 + 框架图 + linter 并行调度 + 结果聚合 + 跑起来 + 偷过来。
