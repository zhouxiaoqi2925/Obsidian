...
|---|---|
| 项目名 | MiroFish |
| 一句话定位 | 基于多智能体图谱的未来预测引擎 |
| 核心问题 | 解决「多 agent 协同 + 时序事件图」领域的核心痛点：基于多智能体图谱的未来预测引擎 |
| 目标用户 | 研究人员 / 数据科学家 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际代码入口**：`frontend/src/main.js`

**核心目录**（文件数最多）：`backend/app/services`, `frontend/src/components`, `backend/app/utils`, `frontend/src/views`, `static/image`, `static/image/Screenshot`
...
**[点状解析]**：AI预测 项目的核心架构看点是 **多 agent 协同 + 时序事件图**。
...
**核心架构看点**（多 agent 协同 + 时序事件图）：
- 多 agent 协同 + 时序事件图
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 AI预测 方向**
- 状态：已采纳
- 背景：解决「多 agent 协同 + 时序事件图」领域的核心痛点：基于多智能体图谱的未来预测引擎
- 决策：采用 多 agent 协同 + 时序事件图 作为核心架构思路
- 理由：该方向在 AI预测 领域已被广泛验证，兼顾性能、可维护性与生态
...
4. `backend/app/services/zep_tools.py`
5. `backend/scripts/run_parallel_simulation.py`
```
...
- **多 agent 协同 + 时序事件图**：这是 MiroFish 的"灵魂"功能，必须精读
- **多 agent 协同 + 时序事件图**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **多 agent 协同 + 时序事件图**（MiroFish 的核心）
   - 实现思路：该方向在 AI预测 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：多 agent 协同 + 时序事件图
   - 自己项目：可借鉴到 开源 + 周边服务

2. **多 agent 协同 + 时序事件图**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 MiroFish 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 frontend/src/main.js → 理解启动流程
- D3: 读核心目录 `backend/app/services`, `frontend/src/components`, `backend/app/utils`, `frontend/src/views`, `static/image`, `static/image/Screenshot` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-MiroFish（只保留核心）
- D6: 把 多 agent 协同 + 时序事件图 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | MiroFish 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 多 agent 协同 + 时序事件图：该方向在 AI预测 领域已被广泛验证，兼顾性能、可维护性与生态
2. 多 agent 协同 + 时序事件图：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. frontend/src/main.js — 启动流程
2. backend/app/services/report_agent.py — 核心实现
3. backend/app/api/simulation.py — 关键算法
4. backend/app/services/simulation_runner.py — 性能优化
5. backend/app/services/zep_tools.py — 边界处理

## 1 个反模式
- AI预测 常见过度设计

## 1 个可复用模式
- 多 agent 协同 + 时序事件图 实现方式

## 我能马上用的 3 件事
1. [ ] 把 多 agent 协同 + 时序事件图 拆成 3 个步骤
2. [ ] 学 多 agent 协同 + 时序事件图 写一个 mini-MiroFish
...
> MiroFish 作为 AI预测 类项目，它的独特看点：

- **多 agent 协同 + 时序事件图** → 该方向在 AI预测 领域已被广泛验证，兼顾性能、可维护性与生态
- **多 agent 协同 + 时序事件图** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 MiroFish = 计划书 + 框架图 + 多 agent 协同 + 时序事件图 + 跑起来 + 偷过来。
