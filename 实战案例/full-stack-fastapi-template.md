...
# full-stack-fastapi-template · 项目深度解析

> FastAPI + React 全栈脚手架
> 来源：项目\full-stack-fastapi-template-master.zip
...
| 一句话定位 | FastAPI + React 全栈脚手架 |
| 核心问题 | 解决「JWT 认证 + SQLModel + Docker」领域的核心痛点：FastAPI + React 全栈脚手架 |
| 目标用户 | 全栈工程师 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。Python/TS 项目常见布局：src/ + lib/ + types/
...
**实际配置入口**：`- `frontend/package.json`
- `package.json`
- `backend/pyproject.toml`
- `pyproject.toml`
- `frontend/tsconfig.json``

**实际代码入口**：`backend/app/api/main.py`

**核心目录**（文件数最多）：`frontend/src/components/ui`, `frontend`, `.github/workflows`, `backend/app`, `frontend/tests`, `frontend/src/components/Common`
...
| 总文件数 | 276 |
| 主语言 | TypeScript |
| 涉及语言 | Markdown, Python, Shell, TypeScript, YAML |
| Star | 34k+ |
| License | MIT License |
| Docker 支持 | ✅ |
| K8s 支持 | ❌ |
...
**[点状解析]**：脚手架 项目的核心架构看点是 **JWT 认证 + SQLModel + Docker**。
...
**核心架构看点**（JWT 认证 + SQLModel + Docker）：
- JWT 认证 + SQLModel + Docker
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 脚手架 方向**
- 状态：已采纳
- 背景：解决「JWT 认证 + SQLModel + Docker」领域的核心痛点：FastAPI + React 全栈脚手架
- 决策：采用 JWT 认证 + SQLModel + Docker 作为核心架构思路
- 理由：该方向在 脚手架 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `backend/tests/api/routes/test_users.py`
2. `frontend/src/client/sdk.gen.ts`
3. `frontend/src/client/schemas.gen.ts`
4. `frontend/src/client/core/request.ts`
5. `frontend/tests/user-settings.spec.ts`
```
...
- **JWT 认证 + SQLModel + Docker**：这是 full-stack-fastapi-template 的"灵魂"功能，必须精读
- **JWT 认证 + SQLModel + Docker**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ❌ |
| Lint 配置 | 见 - `frontend/package.json`
- `package.json`
- `backend/pyproject.toml`
- `pyproject.toml`
- `frontend/tsconfig.json` |
...
| 实践 | full-stack-fastapi-template 怎么做的 | 能不能抄 |
|---|---|---|
...
```markdown
1. **JWT 认证 + SQLModel + Docker**（full-stack-fastapi-template 的核心）
   - 实现思路：该方向在 脚手架 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：JWT 认证 + SQLModel + Docker
   - 自己项目：可借鉴到 开源 + 周边服务

2. **JWT 认证 + SQLModel + Docker**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 full-stack-fastapi-template 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 backend/app/api/main.py → 理解启动流程
- D3: 读核心目录 `frontend/src/components/ui`, `frontend`, `.github/workflows`, `backend/app`, `frontend/tests`, `frontend/src/components/Common` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-full-stack-fastapi-template（只保留核心）
- D6: 把 JWT 认证 + SQLModel + Docker 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | full-stack-fastapi-template 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
```markdown
# 《full-stack-fastapi-template》学习卡片

## 一句话价值
> FastAPI + React 全栈脚手架

## 3 个核心洞察
1. JWT 认证 + SQLModel + Docker：该方向在 脚手架 领域已被广泛验证，兼顾性能、可维护性与生态
2. JWT 认证 + SQLModel + Docker：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. backend/app/api/main.py — 启动流程
2. backend/tests/api/routes/test_users.py — 核心实现
3. frontend/src/client/sdk.gen.ts — 关键算法
4. frontend/src/client/schemas.gen.ts — 性能优化
5. frontend/src/client/core/request.ts — 边界处理

## 1 个反模式
- 脚手架 常见过度设计

## 1 个可复用模式
- JWT 认证 + SQLModel + Docker 实现方式

## 我能马上用的 3 件事
1. [ ] 把 JWT 认证 + SQLModel + Docker 拆成 3 个步骤
2. [ ] 学 JWT 认证 + SQLModel + Docker 写一个 mini-full-stack-fastapi-template
...
> full-stack-fastapi-template 作为 脚手架 类项目，它的独特看点：

- **JWT 认证 + SQLModel + Docker** → 该方向在 脚手架 领域已被广泛验证，兼顾性能、可维护性与生态
- **JWT 认证 + SQLModel + Docker** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
| 字段 | 值 |
|---|---|
| 文件 | 项目\full-stack-fastapi-template-master.zip |
| 大小 | 0.8 MB |
| 总文件 | 276 |
| 解析时间 | 2026-06-01 |
...
> 解析 full-stack-fastapi-template = 计划书 + 框架图 + JWT 认证 + SQLModel + Docker + 跑起来 + 偷过来。
