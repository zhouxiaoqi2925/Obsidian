...
> Rust 小练习集，配套 Rust Book
> 来源：项目\rustlings-main.zip
...
|---|---|
| 项目名 | rustlings |
| 一句话定位 | Rust 小练习集，配套 Rust Book |
| 核心问题 | 解决「渐进式练习 + 编译错误引导」领域的核心痛点：Rust 小练习集，配套 Rust Book |
| 目标用户 | 初学者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**核心目录**（文件数最多）：`src`, `exercises/01_variables`, `exercises/04_primitive_types`, `exercises/13_error_handling`, `exercises/02_functions`, `exercises/06_move_semantics`
...
**[点状解析]**：教学 项目的核心架构看点是 **渐进式练习 + 编译错误引导**。
...
**ADR-001: 为什么是 教学 方向**
- 状态：已采纳
- 背景：解决「渐进式练习 + 编译错误引导」领域的核心痛点：Rust 小练习集，配套 Rust Book
- 决策：采用 渐进式练习 + 编译错误引导 作为核心架构思路
...
- **渐进式练习 + 编译错误引导**：这是 rustlings 的"灵魂"功能，必须精读
- **渐进式练习 + 编译错误引导**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ❌ |
| K8s | ❌ |
| Lint 配置 | 见 - `website/package.json`
- `Cargo.toml`
- `dev-Cargo.toml`
- `dev/Cargo.toml`
- `rustlings-macros/Cargo.toml` |
...
```markdown
## 7 天复刻路径（以 rustlings 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 src/main.rs → 理解启动流程
- D3: 读核心目录 `src`, `exercises/01_variables`, `exercises/04_primitive_types`, `exercises/13_error_handling`, `exercises/02_functions`, `exercises/06_move_semantics` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-rustlings（只保留核心）
- D6: 把 渐进式练习 + 编译错误引导 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | rustlings 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐ |
...
## 3 个核心洞察
1. 渐进式练习 + 编译错误引导：该方向在 教学 领域已被广泛验证，兼顾性能、可维护性与生态
2. 渐进式练习 + 编译错误引导：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. src/main.rs — 启动流程
2. src/app_state.rs — 核心实现
3. src/list/state.rs — 关键算法
4. src/dev/check.rs — 性能优化
5. src/watch/state.rs — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 渐进式练习 + 编译错误引导 拆成 3 个步骤
2. [ ] 学 渐进式练习 + 编译错误引导 写一个 mini-rustlings
...
> rustlings 作为 教学 类项目，它的独特看点：

- **渐进式练习 + 编译错误引导** → 该方向在 教学 领域已被广泛验证，兼顾性能、可维护性与生态
- **渐进式练习 + 编译错误引导** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 rustlings = 计划书 + 框架图 + 渐进式练习 + 编译错误引导 + 跑起来 + 偷过来。
