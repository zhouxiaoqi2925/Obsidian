...
|---|---|
| 项目名 | dubbo |
| 一句话定位 | 阿里出品的高性能 Java RPC |
| 核心问题 | 解决「SPI 扩展 + 多协议 + 服务治理」领域的核心痛点：阿里出品的高性能 Java RPC |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：RPC框架 项目的核心架构看点是 **SPI 扩展 + 多协议 + 服务治理**。
...
**核心架构看点**（SPI 扩展 + 多协议 + 服务治理）：
- SPI 扩展 + 多协议 + 服务治理
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 RPC框架 方向**
- 状态：已采纳
- 背景：解决「SPI 扩展 + 多协议 + 服务治理」领域的核心痛点：阿里出品的高性能 Java RPC
- 决策：采用 SPI 扩展 + 多协议 + 服务治理 作为核心架构思路
- 理由：该方向在 RPC框架 领域已被广泛验证，兼顾性能、可维护性与生态
...
- **SPI 扩展 + 多协议 + 服务治理**：这是 dubbo 的"灵魂"功能，必须精读
- **SPI 扩展 + 多协议 + 服务治理**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **SPI 扩展 + 多协议 + 服务治理**（dubbo 的核心）
   - 实现思路：该方向在 RPC框架 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：SPI 扩展 + 多协议 + 服务治理
   - 自己项目：可借鉴到 开源 + 周边服务

2. **SPI 扩展 + 多协议 + 服务治理**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-dubbo（只保留核心）
- D6: 把 SPI 扩展 + 多协议 + 服务治理 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | dubbo 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. SPI 扩展 + 多协议 + 服务治理：该方向在 RPC框架 领域已被广泛验证，兼顾性能、可维护性与生态
2. SPI 扩展 + 多协议 + 服务治理：核心数据流
...
## 1 个可复用模式
- SPI 扩展 + 多协议 + 服务治理 实现方式

## 我能马上用的 3 件事
1. [ ] 把 SPI 扩展 + 多协议 + 服务治理 拆成 3 个步骤
2. [ ] 学 SPI 扩展 + 多协议 + 服务治理 写一个 mini-dubbo
...
> dubbo 作为 RPC框架 类项目，它的独特看点：

- **SPI 扩展 + 多协议 + 服务治理** → 该方向在 RPC框架 领域已被广泛验证，兼顾性能、可维护性与生态
- **SPI 扩展 + 多协议 + 服务治理** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 dubbo = 计划书 + 框架图 + SPI 扩展 + 多协议 + 服务治理 + 跑起来 + 偷过来。
