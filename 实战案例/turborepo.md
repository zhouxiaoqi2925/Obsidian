...
> turborepo 把 monorepo 的"任务调度 + 增量缓存"做到极致——`turbo.json` pipeline 编译为 DAG，Cap'n Proto 序列化保证跨平台字节级一致哈希，gRPC daemon 复用索引避免冷启动，Vercel 远程缓存让团队共享构建产物。5000 包 monorepo 从"30 分钟"变成"30 秒"。
...
**解决方案**：Turborepo 把 `turbo.json` 定义的 pipeline 任务编译为 DAG，节点是 `package#task`（如 `@app/web#build`），边是 `dependsOn` 显式或隐式（包依赖）。调度器拓扑序执行，相同层级包并行跑。
...
### 模式 13 · 多 Turborepo 实例与跨 monorepo 缓存
...
**3 核心洞察**：
1. 任务图 DAG + 确定性哈希 = monorepo 增量构建的核心范式
2. Cap'n Proto 跨语言序列化 = Rust 引擎与多语言生态字节级一致
3. 写穿缓存（本地 + 远程） = 团队级构建复用 10x 加速

**1 反模式**：缓存键用 `Date.now()` / 随机数——破坏确定性 hash，命中率为 0%。

**3 立刻能用**：
1. `turbo run build --filter=[origin/main]` CI 增量构建
2. `inputs: ["src/**", "package.json"]` 精确控制 hash 输入
3. `TURBO_TOKEN` + Vercel Remote Cache 团队级共享构建产物
