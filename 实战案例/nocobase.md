...
| 项目名 | nocobase |
| 一句话定位 | 可扩展的无代码/低代码平台 |
| 核心问题 | 解决「数据模型驱动 + 区块 + 插件」领域的核心痛点：可扩展的无代码/低代码平台 |
| 目标用户 | 企业 IT / 业务人员 |
| 商业模式 | 商业版授权 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。TS 项目常见布局：src/ + docs/ + tests/
...
- `packages/core/ai/package.json``

**实际代码入口**：`benchmark/koa-database/index.js`

**核心目录**（文件数最多）：`packages/plugins/@nocobase/plugin-ai/src/client/ai-employees/1.x/avatars`, `packages/core/utils/src`, `packages/core/client/src/collection-manager/interfaces`, `packages/core/flow-engine/src/__tests__`, `packages/plugins/@nocobase/plugin-mobile/src/client/demos`, `packages/core/client/docs/zh-CN/core/flow-models/demos`
...
**[点状解析]**：低代码 项目的核心架构看点是 **数据模型驱动 + 区块 + 插件**。
...
**ADR-001: 为什么是 低代码 方向**
- 状态：已采纳
- 背景：解决「数据模型驱动 + 区块 + 插件」领域的核心痛点：可扩展的无代码/低代码平台
- 决策：采用 数据模型驱动 + 区块 + 插件 作为核心架构思路
...
**前 5 个最大源码文件**：
```
1. `packages/core/flow-engine/src/flowContext.ts`
2. `packages/core/flow-engine/src/flowEngine.ts`
3. `packages/core/client/.dumirc.ts`
4. `packages/core/flow-engine/src/flowSettings.ts`
5. `packages/core/server/src/application.ts`
```

**入口文件**：`benchmark/koa-database/index.js`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：benchmark/koa-database/index.js
...
- **数据模型驱动 + 区块 + 插件**：这是 nocobase 的"灵魂"功能，必须精读
- **数据模型驱动 + 区块 + 插件**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
## 7 天复刻路径（以 nocobase 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 benchmark/koa-database/index.js → 理解启动流程
- D3: 读核心目录 `packages/plugins/@nocobase/plugin-ai/src/client/ai-employees/1.x/avatars`, `packages/core/utils/src`, `packages/core/client/src/collection-manager/interfaces`, `packages/core/flow-engine/src/__tests__`, `packages/plugins/@nocobase/plugin-mobile/src/client/demos`, `packages/core/client/docs/zh-CN/core/flow-models/demos` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-nocobase（只保留核心）
- D6: 把 数据模型驱动 + 区块 + 插件 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | nocobase 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 数据模型驱动 + 区块 + 插件：该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
2. 数据模型驱动 + 区块 + 插件：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. benchmark/koa-database/index.js — 启动流程
2. packages/core/flow-engine/src/flowContext.ts — 核心实现
3. packages/core/flow-engine/src/flowEngine.ts — 关键算法
4. packages/core/client/.dumirc.ts — 性能优化
5. packages/core/flow-engine/src/flowSettings.ts — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 数据模型驱动 + 区块 + 插件 拆成 3 个步骤
2. [ ] 学 数据模型驱动 + 区块 + 插件 写一个 mini-nocobase
...
> nocobase 作为 低代码 类项目，它的独特看点：

- **数据模型驱动 + 区块 + 插件** → 该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
- **数据模型驱动 + 区块 + 插件** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 nocobase = 计划书 + 框架图 + 数据模型驱动 + 区块 + 插件 + 跑起来 + 偷过来。
