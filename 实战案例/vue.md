# vue - 渐进式前端框架的 Proxy 响应式、VDOM 双端 diff 与编译时 patch flag 典范
...
> vue 把"渐进式 + 响应式 + 编译时优化"做到极致——Proxy 响应式替代 defineProperty，VDOM 双端 diff 配合 patch flag 让运行时 diff 复杂度接近 O(n)，SFC 编译器把模板静态提升到模块级，Composition API 让逻辑按关注点组织而非按选项类型。Vue 3 + Vite 是当前前端工程化的"事实标准栈"。

## 第一段：基础范式（模式 1-5）

### 模式 1 · Proxy 响应式三件套（reactive/ref/computed）
...
**最佳实践**：基本类型用 `ref`；对象用 `reactive`；`computed` 不传 setter 是只读；用 `markRaw` 标记非响应；大对象用 `shallowRef`。
...
### 模式 3 · SFC（Single File Component）

**问题场景**：组件三段（template/script/style）拆三个文件太碎——HTML 工程师要看 CSS、JS 工程师要看模板，跨段协作难。

**解决方案**：SFC 是 `.vue` 文件，`<template>` + `<script setup>` + `<style scoped>` 三段。Vite / Vue Loader 用 `@vue/compiler-sfc` 拆为三个模块分别编译。`<style scoped>` 自动加 `data-v-xxx` 属性选择器实现局部作用域。
...
**3 核心洞察**：
1. Proxy 替代 defineProperty = 响应式系统更通用（数组/Map/Set 都能拦截）
2. 双端 diff + patch flag + 静态提升 = 运行时 diff 复杂度接近 O(n)
3. Composition API + SFC = 逻辑按关注点组织（vs Options 按选项类型）

**1 反模式**：在 `setup` 外调用 `onMounted` 等生命周期钩子——会因拿不到 `currentInstance` 而抛错。

**3 立刻能用**：
1. `ref` / `reactive` / `computed` 三件套开箱即用
2. `<script setup>` + `defineProps` / `defineEmits` 简化组件
3. `v-memo="[dep]"` + 稳定 `key` 优化长列表
