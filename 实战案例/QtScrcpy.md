...
> Android 实时投屏控制工具（基于 scrcpy）
> 来源：项目\QtScrcpy-dev.zip
...
|---|---|
| 项目名 | QtScrcpy |
| 一句话定位 | Android 实时投屏控制工具（基于 scrcpy） |
| 核心问题 | 解决「ADB 通信 + 视频解码 + 输入注入」领域的核心痛点：Android 实时投屏控制工具（基于 scrcpy） |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。C++/Qt 项目常见布局：include/ + src/ + tests/

**[思维导图]**：
```
投屏工具 框架
├── 2.1 顶层结构（tree -L 2）
├── 2.2 配置入口（CMakeLists.txt / Makefile）
...
**实际配置入口**：`- `ci/mac/package/requirements.txt`
- `CMakeLists.txt`
- `QtScrcpy/CMakeLists.txt`
- `QtScrcpy/res/i18n/CMakeLists.txt``

**实际代码入口**：`QtScrcpy/main.cpp`

**核心目录**（文件数最多）：`QtScrcpy/res/qss/psblack`, `QtScrcpy/res/i18n`, `QtScrcpy/ui`, `QtScrcpy/util/mousetap`, `screenshot`, `QtScrcpy/util`
...
**[点状解析]**：投屏工具 项目的核心架构看点是 **ADB 通信 + 视频解码 + 输入注入**。
...
**核心架构看点**（ADB 通信 + 视频解码 + 输入注入）：
- ADB 通信 + 视频解码 + 输入注入
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 投屏工具 方向**
- 状态：已采纳
- 背景：解决「ADB 通信 + 视频解码 + 输入注入」领域的核心痛点：Android 实时投屏控制工具（基于 scrcpy）
- 决策：采用 ADB 通信 + 视频解码 + 输入注入 作为核心架构思路
- 理由：该方向在 投屏工具 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `QtScrcpy/ui/dialog.cpp`
2. `QtScrcpy/ui/videoform.cpp`
3. `QtScrcpy/util/config.cpp`
4. `QtScrcpy/res/i18n/ja_JP.ts`
5. `QtScrcpy/res/i18n/ko_KR.ts`
```
...
- **ADB 通信 + 视频解码 + 输入注入**：这是 QtScrcpy 的"灵魂"功能，必须精读
- **ADB 通信 + 视频解码 + 输入注入**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ❌ |
| K8s | ❌ |
| Lint 配置 | 见 - `ci/mac/package/requirements.txt`
- `CMakeLists.txt`
- `QtScrcpy/CMakeLists.txt`
- `QtScrcpy/res/i18n/CMakeLists.txt` |
| 性能基准 | 待验证 |
...
**关键配置文件**：`- `ci/mac/package/requirements.txt`
- `CMakeLists.txt`
- `QtScrcpy/CMakeLists.txt`
- `QtScrcpy/res/i18n/CMakeLists.txt``
...
```markdown
1. **ADB 通信 + 视频解码 + 输入注入**（QtScrcpy 的核心）
   - 实现思路：该方向在 投屏工具 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：ADB 通信 + 视频解码 + 输入注入
   - 自己项目：可借鉴到 开源 + 周边服务

2. **ADB 通信 + 视频解码 + 输入注入**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 QtScrcpy 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 QtScrcpy/main.cpp → 理解启动流程
- D3: 读核心目录 `QtScrcpy/res/qss/psblack`, `QtScrcpy/res/i18n`, `QtScrcpy/ui`, `QtScrcpy/util/mousetap`, `screenshot`, `QtScrcpy/util` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-QtScrcpy（只保留核心）
- D6: 把 ADB 通信 + 视频解码 + 输入注入 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | QtScrcpy 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. ADB 通信 + 视频解码 + 输入注入：该方向在 投屏工具 领域已被广泛验证，兼顾性能、可维护性与生态
2. ADB 通信 + 视频解码 + 输入注入：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. QtScrcpy/main.cpp — 启动流程
2. QtScrcpy/ui/dialog.cpp — 核心实现
3. QtScrcpy/ui/videoform.cpp — 关键算法
4. QtScrcpy/util/config.cpp — 性能优化
5. QtScrcpy/res/i18n/ja_JP.ts — 边界处理

## 1 个反模式
- 投屏工具 常见过度设计

## 1 个可复用模式
- ADB 通信 + 视频解码 + 输入注入 实现方式

## 我能马上用的 3 件事
1. [ ] 把 ADB 通信 + 视频解码 + 输入注入 拆成 3 个步骤
2. [ ] 学 ADB 通信 + 视频解码 + 输入注入 写一个 mini-QtScrcpy
...
> QtScrcpy 作为 投屏工具 类项目，它的独特看点：

- **ADB 通信 + 视频解码 + 输入注入** → 该方向在 投屏工具 领域已被广泛验证，兼顾性能、可维护性与生态
- **ADB 通信 + 视频解码 + 输入注入** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 QtScrcpy = 计划书 + 框架图 + ADB 通信 + 视频解码 + 输入注入 + 跑起来 + 偷过来。
