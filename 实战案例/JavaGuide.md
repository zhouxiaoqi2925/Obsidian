...
|---|---|
| 项目名 | JavaGuide |
| 一句话定位 | Java 程序员面试突击手册 |
| 核心问题 | 解决「知识图谱 + 分类体系」领域的核心痛点：Java 程序员面试突击手册 |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。Java 项目常见布局：src/ + docs/ + tests/
...
**实际代码入口**：`docs/.vuepress/sidebar/index.ts`

**核心目录**（文件数最多）：`docs/java/concurrent/images/thread-local`, `docs/cs-basics/network`, `docs/cs-basics/data-structure/pictures/堆`, `docs/java/new-features`, `docs/java/concurrent`, `docs/database/mysql`
...
**[点状解析]**：面试指南 项目的核心架构看点是 **知识图谱 + 分类体系**。
...
**ADR-001: 为什么是 面试指南 方向**
- 状态：已采纳
- 背景：解决「知识图谱 + 分类体系」领域的核心痛点：Java 程序员面试突击手册
- 决策：采用 知识图谱 + 分类体系 作为核心架构思路
- 理由：该方向在 面试指南 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `docs/.vuepress/sidebar/index.ts`
2. `docs/.vuepress/theme.ts`
3. `docs/.vuepress/sidebar/cs-basics.ts`
4. `sw.js`
5. `docs/.vuepress/sidebar/ai.ts`
```

**入口文件**：`docs/.vuepress/sidebar/index.ts`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：docs/.vuepress/sidebar/index.ts
...
- **知识图谱 + 分类体系**：这是 JavaGuide 的"灵魂"功能，必须精读
- **知识图谱 + 分类体系**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **知识图谱 + 分类体系**（JavaGuide 的核心）
   - 实现思路：该方向在 面试指南 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：知识图谱 + 分类体系
   - 自己项目：可借鉴到 开源 + 周边服务
...
```markdown
## 7 天复刻路径（以 JavaGuide 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 docs/.vuepress/sidebar/index.ts → 理解启动流程
- D3: 读核心目录 `docs/java/concurrent/images/thread-local`, `docs/cs-basics/network`, `docs/cs-basics/data-structure/pictures/堆`, `docs/java/new-features`, `docs/java/concurrent`, `docs/database/mysql` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-JavaGuide（只保留核心）
- D6: 把 知识图谱 + 分类体系 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | JavaGuide 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 知识图谱 + 分类体系：该方向在 面试指南 领域已被广泛验证，兼顾性能、可维护性与生态
2. 知识图谱 + 分类体系：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. docs/.vuepress/sidebar/index.ts — 启动流程
2. docs/.vuepress/sidebar/index.ts — 核心实现
3. docs/.vuepress/theme.ts — 关键算法
4. docs/.vuepress/sidebar/cs-basics.ts — 性能优化
5. sw.js — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 知识图谱 + 分类体系 拆成 3 个步骤
2. [ ] 学 知识图谱 + 分类体系 写一个 mini-JavaGuide
...
> JavaGuide 作为 面试指南 类项目，它的独特看点：

- **知识图谱 + 分类体系** → 该方向在 面试指南 领域已被广泛验证，兼顾性能、可维护性与生态
- **知识图谱 + 分类体系** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 JavaGuide = 计划书 + 框架图 + 知识图谱 + 分类体系 + 跑起来 + 偷过来。
