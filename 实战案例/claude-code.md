...
> Anthropic 官方的终端 AI 编程助手
> 来源：项目\claude-code-main.zip
...
|---|---|
| 项目名 | claude-code |
| 一句话定位 | Anthropic 官方的终端 AI 编程助手 |
| 核心问题 | 解决「工具调用 + Agent 循环」领域的核心痛点：Anthropic 官方的终端 AI 编程助手 |
| 目标用户 | AI 应用开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**核心目录**（文件数最多）：`.github/workflows`, `plugins/security-guidance/hooks`, `scripts`, `plugins/plugin-dev/skills/command-development/references`, `plugins/hookify/hooks`, `plugins/pr-review-toolkit/agents`
...
| 总文件数 | 303 |
| 主语言 | Markdown |
| 涉及语言 | Markdown, Python, Shell, YAML |
| Star | — |
...
**[点状解析]**：AI Agent 项目的核心架构看点是 **工具调用 + Agent 循环**。
...
- 状态：已采纳
- 背景：解决「工具调用 + Agent 循环」领域的核心痛点：Anthropic 官方的终端 AI 编程助手
- 决策：采用 工具调用 + Agent 循环 作为核心架构思路
...
**前 5 个最大源码文件**：
```
1. `plugins/security-guidance/hooks/llm.py`
2. `plugins/security-guidance/hooks/security_reminder_hook.py`
3. `plugins/security-guidance/hooks/gitutil.py`
4. `plugins/security-guidance/hooks/review_api.py`
5. `plugins/security-guidance/hooks/diffstate.py`
```
...
- **工具调用 + Agent 循环**：这是 claude-code 的"灵魂"功能，必须精读
- **工具调用 + Agent 循环**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **工具调用 + Agent 循环**（claude-code 的核心）
   - 实现思路：该方向在 AI Agent 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：工具调用 + Agent 循环
   - 自己项目：可借鉴到 开源 + 周边服务

2. **工具调用 + Agent 循环**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 claude-code 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 `.github/workflows`, `plugins/security-guidance/hooks`, `scripts`, `plugins/plugin-dev/skills/command-development/references`, `plugins/hookify/hooks`, `plugins/pr-review-toolkit/agents` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-claude-code（只保留核心）
- D6: 把 工具调用 + Agent 循环 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | claude-code 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 工具调用 + Agent 循环：该方向在 AI Agent 领域已被广泛验证，兼顾性能、可维护性与生态
2. 工具调用 + Agent 循环：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1.  — 启动流程
2. plugins/security-guidance/hooks/llm.py — 核心实现
3. plugins/security-guidance/hooks/security_reminder_hook.py — 关键算法
4. plugins/security-guidance/hooks/gitutil.py — 性能优化
5. plugins/security-guidance/hooks/review_api.py — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 工具调用 + Agent 循环 拆成 3 个步骤
2. [ ] 学 工具调用 + Agent 循环 写一个 mini-claude-code
...
> claude-code 作为 AI Agent 类项目，它的独特看点：

- **工具调用 + Agent 循环** → 该方向在 AI Agent 领域已被广泛验证，兼顾性能、可维护性与生态
- **工具调用 + Agent 循环** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 claude-code = 计划书 + 框架图 + 工具调用 + Agent 循环 + 跑起来 + 偷过来。
