...
| 项目名 | DooTask |
| 一句话定位 | 轻量级在线项目协同工具 |
| 核心问题 | 解决「IM + 任务 + 文档 三合一」领域的核心痛点：轻量级在线项目协同工具 |
| 目标用户 | 团队 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。PHP/Go 项目常见布局：src/ + lib/ + bin/
...
**实际配置入口**：`- `electron/package.json`
- `package.json`
- `composer.json`
- `language/composer.json``

**实际代码入口**：`public/docs/main.js`

**核心目录**（文件数最多）：`public/js/build`, `public/js/ace`, `resources/assets/statics/public/js/ace`, `database/migrations`, `public/js/ace/snippets`, `resources/assets/statics/public/js/ace/snippets`
...
| 总文件数 | 4324 |
| 主语言 | JavaScript |
| 涉及语言 | JavaScript, Markdown, PHP, Vue |
| Star | 13k+ |
...
**[点状解析]**：项目管理 项目的核心架构看点是 **IM + 任务 + 文档 三合一**。
...
**ADR-001: 为什么是 项目管理 方向**
- 状态：已采纳
- 背景：解决「IM + 任务 + 文档 三合一」领域的核心痛点：轻量级在线项目协同工具
- 决策：采用 IM + 任务 + 文档 三合一 作为核心架构思路
- 理由：该方向在 项目管理 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `public/js/build/manage.766b6dc5.js`
2. `resources/assets/js/store/actions.js`
3. `public/js/build/project.a8a2dbee.js`
4. `public/js/build/tui-calendar-hi.8947f8cc.js`
5. `app/Http/Controllers/Api/ProjectController.php`
```

**入口文件**：`public/docs/main.js`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：public/docs/main.js
...
- **IM + 任务 + 文档 三合一**：这是 DooTask 的"灵魂"功能，必须精读
- **IM + 任务 + 文档 三合一**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ❌ |
| Lint 配置 | 见 - `electron/package.json`
- `package.json`
- `composer.json`
- `language/composer.json` |
| 性能基准 | 待验证 |
...
```markdown
1. **IM + 任务 + 文档 三合一**（DooTask 的核心）
   - 实现思路：该方向在 项目管理 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：IM + 任务 + 文档 三合一
   - 自己项目：可借鉴到 开源 + 周边服务
...
```markdown
## 7 天复刻路径（以 DooTask 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 public/docs/main.js → 理解启动流程
- D3: 读核心目录 `public/js/build`, `public/js/ace`, `resources/assets/statics/public/js/ace`, `database/migrations`, `public/js/ace/snippets`, `resources/assets/statics/public/js/ace/snippets` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-DooTask（只保留核心）
- D6: 把 IM + 任务 + 文档 三合一 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | DooTask 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. IM + 任务 + 文档 三合一：该方向在 项目管理 领域已被广泛验证，兼顾性能、可维护性与生态
2. IM + 任务 + 文档 三合一：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. public/docs/main.js — 启动流程
2. public/js/build/manage.766b6dc5.js — 核心实现
3. resources/assets/js/store/actions.js — 关键算法
4. public/js/build/project.a8a2dbee.js — 性能优化
5. public/js/build/tui-calendar-hi.8947f8cc.js — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 IM + 任务 + 文档 三合一 拆成 3 个步骤
2. [ ] 学 IM + 任务 + 文档 三合一 写一个 mini-DooTask
...
> DooTask 作为 项目管理 类项目，它的独特看点：

- **IM + 任务 + 文档 三合一** → 该方向在 项目管理 领域已被广泛验证，兼顾性能、可维护性与生态
- **IM + 任务 + 文档 三合一** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 DooTask = 计划书 + 框架图 + IM + 任务 + 文档 三合一 + 跑起来 + 偷过来。
