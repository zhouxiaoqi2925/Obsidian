...
| 项目名 | strategies |
| 一句话定位 | 量化交易策略集合 |
| 核心问题 | 解决「策略回测 + 数据处理」领域的核心痛点：量化交易策略集合 |
| 目标用户 | 量化 / 金融 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。Python 项目常见布局：src/ + docs/ + tests/
...
```
├── /
├── 1-2-3-形态与-EMAMACD-和第四根蜡烛线延伸的量化交易策略1-2-3-Pattern-Quantitative-Trading-Strategy-with-EMAs-MACD-and-4th-Candle-Extension.md/
├── 1-3-1-红绿K线反转策略1-3-1-Red-Green-Candlestick-Reversal-Strategy.md/
├── 10EMA双重交叉趋势追踪策略10EMA-Double-Cross-Trend-Tracking-Strategy.md/
├── 10SMA与MACD双重趋势跟踪交易策略-10SMA-and-MACD-Dual-Trend-Following-Trading-Strategy.md/
├── 123反转与STARC波段组合策略123-Reversal-and-STARC-Bands-Combo-Strategy.md/
├── 123反转与费希尔变换指标组合策略123-Reversal-and-Fisher-Transform-Indicator-Combo-Strategy.md/
├── 15MIN-BTCUSDTPERP-BOT.md/
├── 15分钟图表吞没突破多重确认策略-15-Minute-Engulfing-Breakout-Multi-Confirmation-Strategy.md/
├── 15分钟突破多周期协同策略基于风险收益比优化模型15-Minute-Breakout-Multi-Timeframe-Synergy-Strategy-with-Risk-Reward-Optimization-Model.md/
├── 1btc起Fmex空头解锁策略后市看空.md/
├── 1btc起fmex多头解锁策略后市看多.md/
├── 2-20指数移动平均策略2-20-Exponential-Moving-Average-Strategy.md/
├── 2-Moving-Average-Color-Direction-Detection.md/
├── 200均线VWAPMFI趋势跟踪策略-200-EMA-VWAP-MFI-Trend-Following-Strategy.md/
├── 2014年的简易比特币高频策略机器人.md/
├── 20均线趋势突破量化交易策略-EMA-20-Breakout-Trend-Following-Quantitative-Trading-Strategy.md/
├── 20水平突破策略20-Level-Breakout-Strategy.md/
├── 24小时量价结合斐波那契回调交叉均线交易策略-24-Hour-Volume-Price-Fibonacci-Retracement-MA-Cross-Trading-Strategy.md/
├── 3-10震荡器轮廓标记策略3-10-Oscillator-Profile-Flagging-Strategy.md/
```
...
**ADR-001: 为什么是 金融 方向**
- 状态：已采纳
- 背景：解决「策略回测 + 数据处理」领域的核心痛点：量化交易策略集合
- 决策：采用 策略回测 + 数据处理 作为核心架构思路
- 理由：该方向在 金融 领域已被广泛验证，兼顾性能、可维护性与生态
...
- **策略回测 + 数据处理**：这是 strategies 的"灵魂"功能，必须精读
- **策略回测 + 数据处理**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **策略回测 + 数据处理**（strategies 的核心）
   - 实现思路：该方向在 金融 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：策略回测 + 数据处理
   - 自己项目：可借鉴到 开源 + 周边服务
...
```markdown
## 7 天复刻路径（以 strategies 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-strategies（只保留核心）
- D6: 把 策略回测 + 数据处理 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | strategies 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 策略回测 + 数据处理：该方向在 金融 领域已被广泛验证，兼顾性能、可维护性与生态
2. 策略回测 + 数据处理：核心数据流
...
## 我能马上用的 3 件事
1. [ ] 把 策略回测 + 数据处理 拆成 3 个步骤
2. [ ] 学 策略回测 + 数据处理 写一个 mini-strategies
...
> strategies 作为 金融 类项目，它的独特看点：

- **策略回测 + 数据处理** → 该方向在 金融 领域已被广泛验证，兼顾性能、可维护性与生态
- **策略回测 + 数据处理** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 strategies = 计划书 + 框架图 + 策略回测 + 数据处理 + 跑起来 + 偷过来。
