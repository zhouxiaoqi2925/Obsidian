...
**解决方案**：LCEL（LangChain Expression Language）支持 `prompt | model | parser` 用 `|` 串成 RunnableSequence；`chain.invoke(input)` 一步跑完 3 步；`chain.batch([i1, i2, i3])` 批量；`chain.stream(input)` 流式；4 种方法自动组合。
...
**解决方案**：30 行 smoke test 验证 3 件套：```python
from langchain.chat_models import init_chat_model
from langchain_core.prompts import ChatPromptTemplate
from langchain_core.output_parsers import StrOutputParser

model = init_chat_model("openai:gpt-5.4")
prompt = ChatPromptTemplate.from_messages([("system", "You are helpful"), ("user", "{q}")])
chain = prompt | model | StrOutputParser()
print(chain.invoke({"q": "Hello"}))
``` 期望：模型返回 "Hello! How can I help you?"。
...
- **大小**: ~50 MB
- **核心仓库**: langchain / langchain-core / langchain-openai / langchain-anthropic 等
- **关键 commit**: v1.x（Runnable 协议稳定期）
- **作者**: Harrison Chase + LangChain 团队 20+ 工程师
- **许可**: MIT
- **被采用**: 月下载 6000 万+，1 万+ 公司生产

## 一句话总结

LangChain 用 Python 把"Runnable 协议 + LCEL 链式 + 700+ partner 集成 + LangGraph 状态机"做到极致，95k+ Star 是 LLM 应用工程事实标准，Runnable 协议是 AI 时代的 API 标杆。
