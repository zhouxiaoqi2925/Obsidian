# langchain - LLM 应用工程框架（Runnable 协议）
...
**解决方案**：30 行 smoke test 验证 3 件套：`from langchain.chat_models import init_chat_model; from langchain_core.prompts import ChatPromptTemplate; from langchain_core.output_parsers import StrOutputParser; model = init_chat_model("openai:gpt-5.4"); prompt = ChatPromptTemplate.from_messages([("system", "You are helpful"), ("user", "{q}")]); chain = prompt | model | StrOutputParser(); print(chain.invoke({"q": "Hello"}))`。期望：模型返回 "Hello! How can I help you?"。
