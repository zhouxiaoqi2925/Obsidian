...
|---|---|
| 项目名 | FastGPT |
| 一句话定位 | 知识库 RAG 平台，开箱即用 |
| 核心问题 | 解决「向量检索 + 工作流编排」领域的核心痛点：知识库 RAG 平台，开箱即用 |
| 目标用户 | AI 应用开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际配置入口**：`- `document/package.json`
- `package.json`
- `packages/global/package.json`
- `packages/service/package.json`
- `packages/web/package.json``

**实际代码入口**：`scripts/i18n/index.js`

**核心目录**（文件数最多）：`document/public/imgs`, `document/content/self-host/upgrading/outdated`, `packages/web/components/common/Icon/icons/common`, `packages/web/components/common/Icon/icons/core/workflow/template`, `packages/web/components/common/Icon/icons`, `document/content/self-host/upgrading/4-14`
...
**[点状解析]**：LLM平台 项目的核心架构看点是 **向量检索 + 工作流编排**。
...
- 状态：已采纳
- 背景：解决「向量检索 + 工作流编排」领域的核心痛点：知识库 RAG 平台，开箱即用
- 决策：采用 向量检索 + 工作流编排 作为核心架构思路
...
**前 5 个最大源码文件**：
```
1. `projects/code-sandbox/test/unit/security.test.ts`
2. `projects/code-sandbox/test/integration/functional.test.ts`
3. `packages/service/core/chat/saveChat.ts`
4. `projects/code-sandbox/test/unit/process-pool.test.ts`
5. `projects/code-sandbox/test/unit/resource-limits.test.ts`
```

**入口文件**：`scripts/i18n/index.js`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：scripts/i18n/index.js
...
- **向量检索 + 工作流编排**：这是 FastGPT 的"灵魂"功能，必须精读
- **向量检索 + 工作流编排**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ✅ |
| Lint 配置 | 见 - `document/package.json`
- `package.json`
- `packages/global/package.json`
- `packages/service/package.json`
- `packages/web/package.json` |
...
```markdown
## 7 天复刻路径（以 FastGPT 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 scripts/i18n/index.js → 理解启动流程
- D3: 读核心目录 `document/public/imgs`, `document/content/self-host/upgrading/outdated`, `packages/web/components/common/Icon/icons/common`, `packages/web/components/common/Icon/icons/core/workflow/template`, `packages/web/components/common/Icon/icons`, `document/content/self-host/upgrading/4-14` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-FastGPT（只保留核心）
- D6: 把 向量检索 + 工作流编排 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | FastGPT 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 向量检索 + 工作流编排：该方向在 LLM平台 领域已被广泛验证，兼顾性能、可维护性与生态
2. 向量检索 + 工作流编排：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. scripts/i18n/index.js — 启动流程
2. projects/code-sandbox/test/unit/security.test.ts — 核心实现
3. projects/code-sandbox/test/integration/functional.test.ts — 关键算法
4. packages/service/core/chat/saveChat.ts — 性能优化
5. projects/code-sandbox/test/unit/process-pool.test.ts — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 向量检索 + 工作流编排 拆成 3 个步骤
2. [ ] 学 向量检索 + 工作流编排 写一个 mini-FastGPT
...
> FastGPT 作为 LLM平台 类项目，它的独特看点：

- **向量检索 + 工作流编排** → 该方向在 LLM平台 领域已被广泛验证，兼顾性能、可维护性与生态
- **向量检索 + 工作流编排** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 FastGPT = 计划书 + 框架图 + 向量检索 + 工作流编排 + 跑起来 + 偷过来。
