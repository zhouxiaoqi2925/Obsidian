...
|---|---|
| 项目名 | mycat |
| 一句话定位 | 基于 Proxy 的 MySQL 中间件 |
| 核心问题 | 解决「SQL 解析 + 路由 + 分片」领域的核心痛点：基于 Proxy 的 MySQL 中间件 |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：数据库中间件 项目的核心架构看点是 **SQL 解析 + 路由 + 分片**。
...
**ADR-001: 为什么是 数据库中间件 方向**
- 状态：已采纳
- 背景：解决「SQL 解析 + 路由 + 分片」领域的核心痛点：基于 Proxy 的 MySQL 中间件
- 决策：采用 SQL 解析 + 路由 + 分片 作为核心架构思路
- 理由：该方向在 数据库中间件 领域已被广泛验证，兼顾性能、可维护性与生态
...
- **SQL 解析 + 路由 + 分片**：这是 mycat 的"灵魂"功能，必须精读
- **SQL 解析 + 路由 + 分片**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **SQL 解析 + 路由 + 分片**（mycat 的核心）
   - 实现思路：该方向在 数据库中间件 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：SQL 解析 + 路由 + 分片
   - 自己项目：可借鉴到 开源 + 周边服务
...
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-mycat（只保留核心）
- D6: 把 SQL 解析 + 路由 + 分片 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | mycat 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. SQL 解析 + 路由 + 分片：该方向在 数据库中间件 领域已被广泛验证，兼顾性能、可维护性与生态
2. SQL 解析 + 路由 + 分片：核心数据流
...
## 我能马上用的 3 件事
1. [ ] 把 SQL 解析 + 路由 + 分片 拆成 3 个步骤
2. [ ] 学 SQL 解析 + 路由 + 分片 写一个 mini-mycat
...
> mycat 作为 数据库中间件 类项目，它的独特看点：

- **SQL 解析 + 路由 + 分片** → 该方向在 数据库中间件 领域已被广泛验证，兼顾性能、可维护性与生态
- **SQL 解析 + 路由 + 分片** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践

**与同类的对比**：
vs 其他 数据库中间件 方案：特点突出
...
> 解析 mycat = 计划书 + 框架图 + SQL 解析 + 路由 + 分片 + 跑起来 + 偷过来。
