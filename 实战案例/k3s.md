...
|---|---|
| 项目名 | k3s |
| 一句话定位 | Rancher 出品的轻量 K8s，单二进制 |
| 核心问题 | 解决「SQLite 默认 + 边缘场景 + 简化安装」领域的核心痛点：Rancher 出品的轻量 K8s，单二进制 |
| 目标用户 | SRE / DevOps |
...
**[点状解析]**：容器编排 项目的核心架构看点是 **SQLite 默认 + 边缘场景 + 简化安装**。
...
- 状态：已采纳
- 背景：解决「SQLite 默认 + 边缘场景 + 简化安装」领域的核心痛点：Rancher 出品的轻量 K8s，单二进制
- 决策：采用 SQLite 默认 + 边缘场景 + 简化安装 作为核心架构思路
...
- **SQLite 默认 + 边缘场景 + 简化安装**：这是 k3s 的"灵魂"功能，必须精读
...
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-k3s（只保留核心）
- D6: 把 SQLite 默认 + 边缘场景 + 简化安装 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | k3s 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. SQLite 默认 + 边缘场景 + 简化安装：该方向在 容器编排 领域已被广泛验证，兼顾性能、可维护性与生态
...
## 1 个可复用模式
- SQLite 默认 + 边缘场景 + 简化安装 实现方式

## 我能马上用的 3 件事
1. [ ] 把 SQLite 默认 + 边缘场景 + 简化安装 拆成 3 个步骤
2. [ ] 学 controller 模式 + reconcile loop 写一个 mini-k3s
...
> k3s 作为 容器编排 类项目，它的独特看点：

- **SQLite 默认 + 边缘场景 + 简化安装** → 该方向在 容器编排 领域已被广泛验证，兼顾性能、可维护性与生态
...
> 解析 k3s = 计划书 + 框架图 + SQLite 默认 + 边缘场景 + 简化安装 + 跑起来 + 偷过来。
