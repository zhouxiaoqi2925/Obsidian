...
|---|---|
| 项目名 | tidb |
| 一句话定位 | MySQL 兼容的分布式 HTAP 数据库 |
| 核心问题 | 解决「Placement Driver + TiKV 存储层 + SQL 层」领域的核心痛点：MySQL 兼容的分布式 HTAP 数据库 |
| 目标用户 | DBA / 后端架构 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际代码入口**：`br/cmd/br/main.go`

**核心目录**（文件数最多）：`pkg/executor`, `pkg/expression`, `pkg/ddl`, `docs/design`, `pkg/planner/core`, `docs/design/imgs`
...
| 总文件数 | 8858 |
| 主语言 | Go |
| 涉及语言 | Go, Markdown, SQL, Shell, YAML |
| Star | 38k+ |
...
**[点状解析]**：分布式SQL 项目的核心架构看点是 **Placement Driver + TiKV 存储层 + SQL 层**。
...
**核心架构看点**（Placement Driver + TiKV 存储层 + SQL 层）：
- PD (Placement Driver) + TiKV
- SQL 层 + 优化器
- MVCC + Raft 副本组

**ADR-001: 为什么是 分布式SQL 方向**
- 状态：已采纳
- 背景：解决「Placement Driver + TiKV 存储层 + SQL 层」领域的核心痛点：MySQL 兼容的分布式 HTAP 数据库
- 决策：采用 Placement Driver + TiKV 存储层 + SQL 层 作为核心架构思路
- 理由：该方向在 分布式SQL 领域已被广泛验证，兼顾性能、可维护性与生态
...
2. `pkg/ddl/partition.go`
3. `pkg/ddl/tests/partition/db_partition_test.go`
4. `pkg/infoschema/metric_table_def.go`
5. `pkg/infoschema/tables.go`
```

**入口文件**：`br/cmd/br/main.go`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：br/cmd/br/main.go
...
- **Placement Driver + TiKV 存储层 + SQL 层**：这是 tidb 的"灵魂"功能，必须精读
- **PD (Placement Driver) + TiKV**：核心架构创新
- **SQL 层 + 优化器**：性能/可用性关键
...
```markdown
1. **Placement Driver + TiKV 存储层 + SQL 层**（tidb 的核心）
   - 实现思路：该方向在 分布式SQL 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：PD (Placement Driver) + TiKV
   - 自己项目：可借鉴到 开源 + 周边服务

2. **PD (Placement Driver) + TiKV**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
## 7 天复刻路径（以 tidb 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 br/cmd/br/main.go → 理解启动流程
- D3: 读核心目录 `pkg/executor`, `pkg/expression`, `pkg/ddl`, `docs/design`, `pkg/planner/core`, `docs/design/imgs` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-tidb（只保留核心）
- D6: 把 Placement Driver + TiKV 存储层 + SQL 层 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | tidb 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. Placement Driver + TiKV 存储层 + SQL 层：该方向在 分布式SQL 领域已被广泛验证，兼顾性能、可维护性与生态
2. PD (Placement Driver) + TiKV：SQL 层 + 优化器
3. MVCC + Raft 副本组：可直接借鉴到自己的项目

## 5 段必读代码
1. br/cmd/br/main.go — 启动流程
2. pkg/session/session.go — 核心实现
3. pkg/ddl/partition.go — 关键算法
4. pkg/ddl/tests/partition/db_partition_test.go — 性能优化
5. pkg/infoschema/metric_table_def.go — 边界处理

## 1 个反模式
- 分布式SQL 常见过度设计

## 1 个可复用模式
- Placement Driver + TiKV 存储层 + SQL 层 实现方式

## 我能马上用的 3 件事
1. [ ] 把 Placement Driver + TiKV 存储层 + SQL 层 拆成 3 个步骤
2. [ ] 学 PD (Placement Driver) + TiKV 写一个 mini-tidb
3. [ ] 把 SQL 层 + 优化器 用到自己的 开源 + 周边服务
```
...
> tidb 作为 分布式SQL 类项目，它的独特看点：

- **Placement Driver + TiKV 存储层 + SQL 层** → 该方向在 分布式SQL 领域已被广泛验证，兼顾性能、可维护性与生态
- **PD (Placement Driver) + TiKV** → SQL 层 + 优化器
- **MVCC + Raft 副本组** → 可借鉴的工程实践

**与同类的对比**：
vs CockroachDB / YugabyteDB：MySQL 兼容 + HTAP
...
> 解析 tidb = 计划书 + 框架图 + Placement Driver + TiKV 存储层 + SQL 层 + 跑起来 + 偷过来。
