...
> BPMN 2.0 流程引擎
> 来源：项目\flowable-engine-main.zip
...
| 项目名 | flowable |
| 一句话定位 | BPMN 2.0 流程引擎 |
| 核心问题 | 解决「BPMN 解析 + 流程执行 + 任务调度」领域的核心痛点：BPMN 2.0 流程引擎 |
| 目标用户 | 企业 IT |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际配置入口**：`- `docs/docusaurus/website/package.json`
- `build-tools/pom.xml`
- `distro/pom.xml`
...
**核心目录**（文件数最多）：`modules/flowable-engine/src/main/resources/org/flowable/db/upgrade`, `modules/flowable-cmmn-engine/src/test/resources/org/flowable/cmmn/test/runtime`, `modules/flowable-engine-common/src/main/resources/org/flowable/common/db/upgrade`, `modules/flowable-engine/src/main/java/org/flowable/engine/impl/cmd`, `docs/docusaurus/docs/assets/static/userguide-5/images`, `docs/docusaurus/docs/assets/bpmn`
...
| 总文件数 | 19441 |
| 主语言 | Java |
| 涉及语言 | Java, JavaScript, Markdown, SQL, Shell, YAML |
| Star | 8.5k+ |
...
**[点状解析]**：工作流 项目的核心架构看点是 **BPMN 解析 + 流程执行 + 任务调度**。
...
**ADR-001: 为什么是 工作流 方向**
- 状态：已采纳
- 背景：解决「BPMN 解析 + 流程执行 + 任务调度」领域的核心痛点：BPMN 2.0 流程引擎
- 决策：采用 BPMN 解析 + 流程执行 + 任务调度 作为核心架构思路
- 理由：该方向在 工作流 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `docs/docusaurus/website/siteConfig.js`
2. `docs/docusaurus/website/core/Footer.js`
```
...
- **BPMN 解析 + 流程执行 + 任务调度**：这是 flowable 的"灵魂"功能，必须精读
- **BPMN 2.0 解析**：核心架构创新
- **流程实例执行 + 任务调度**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ✅ |
| Lint 配置 | 见 - `docs/docusaurus/website/package.json`
- `build-tools/pom.xml`
- `distro/pom.xml`
- `docs/public-api/tools/flowable-oas-generator/pom.xml`
- `docs/public-api/tools/flowable-rest-asciidoc/pom.xml` |
...
**关键配置文件**：`- `docs/docusaurus/website/package.json`
- `build-tools/pom.xml`
- `distro/pom.xml`
- `docs/public-api/tools/flowable-oas-generator/pom.xml`
- `docs/public-api/tools/flowable-rest-asciidoc/pom.xml``
...
```markdown
1. **BPMN 解析 + 流程执行 + 任务调度**（flowable 的核心）
   - 实现思路：该方向在 工作流 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：BPMN 2.0 解析
   - 自己项目：可借鉴到 开源 + 周边服务
...
```markdown
## 7 天复刻路径（以 flowable 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 `modules/flowable-engine/src/main/resources/org/flowable/db/upgrade`, `modules/flowable-cmmn-engine/src/test/resources/org/flowable/cmmn/test/runtime`, `modules/flowable-engine-common/src/main/resources/org/flowable/common/db/upgrade`, `modules/flowable-engine/src/main/java/org/flowable/engine/impl/cmd`, `docs/docusaurus/docs/assets/static/userguide-5/images`, `docs/docusaurus/docs/assets/bpmn` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-flowable（只保留核心）
- D6: 把 BPMN 解析 + 流程执行 + 任务调度 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | flowable 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. BPMN 解析 + 流程执行 + 任务调度：该方向在 工作流 领域已被广泛验证，兼顾性能、可维护性与生态
2. BPMN 2.0 解析：流程实例执行 + 任务调度
3. 历史数据 + 回滚：可直接借鉴到自己的项目

## 5 段必读代码
1.  — 启动流程
2. docs/docusaurus/website/siteConfig.js — 核心实现
3. docs/docusaurus/website/core/Footer.js — 关键算法
4.  — 性能优化
5.  — 边界处理
...
## 1 个可复用模式
- BPMN 解析 + 流程执行 + 任务调度 实现方式

## 我能马上用的 3 件事
1. [ ] 把 BPMN 解析 + 流程执行 + 任务调度 拆成 3 个步骤
2. [ ] 学 BPMN 2.0 解析 写一个 mini-flowable
3. [ ] 把 流程实例执行 + 任务调度 用到自己的 开源 + 周边服务
```
...
> flowable 作为 工作流 类项目，它的独特看点：

- **BPMN 解析 + 流程执行 + 任务调度** → 该方向在 工作流 领域已被广泛验证，兼顾性能、可维护性与生态
- **BPMN 2.0 解析** → 流程实例执行 + 任务调度
- **历史数据 + 回滚** → 可借鉴的工程实践

**与同类的对比**：
vs Camunda / Activiti：用户友好 + 云原生
...
| 字段 | 值 |
|---|---|
| 文件 | 项目\flowable-engine-main.zip |
| 大小 | 64.7 MB |
| 总文件 | 19441 |
| 解析时间 | 2026-06-01 |
...
> 解析 flowable = 计划书 + 框架图 + BPMN 解析 + 流程执行 + 任务调度 + 跑起来 + 偷过来。
