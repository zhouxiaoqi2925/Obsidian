...
|---|---|
| 项目名 | sentinel |
| 一句话定位 | 阿里出品的流量控制 + 熔断 + 系统自适应 |
| 核心问题 | 解决「滑动窗口 + 规则 + 控制台」领域的核心痛点：阿里出品的流量控制 + 熔断 + 系统自适应 |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：流量控制 项目的核心架构看点是 **滑动窗口 + 规则 + 控制台**。
...
**ADR-001: 为什么是 流量控制 方向**
- 状态：已采纳
- 背景：解决「滑动窗口 + 规则 + 控制台」领域的核心痛点：阿里出品的流量控制 + 熔断 + 系统自适应
- 决策：采用 滑动窗口 + 规则 + 控制台 作为核心架构思路
- 理由：该方向在 流量控制 领域已被广泛验证，兼顾性能、可维护性与生态
...
- **滑动窗口 + 规则 + 控制台**：这是 sentinel 的"灵魂"功能，必须精读
- **滑动窗口 + 规则 + 控制台**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **滑动窗口 + 规则 + 控制台**（sentinel 的核心）
   - 实现思路：该方向在 流量控制 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：滑动窗口 + 规则 + 控制台
   - 自己项目：可借鉴到 开源 + 周边服务
...
```markdown
## 7 天复刻路径（以 sentinel 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-sentinel（只保留核心）
- D6: 把 滑动窗口 + 规则 + 控制台 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | sentinel 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 滑动窗口 + 规则 + 控制台：该方向在 流量控制 领域已被广泛验证，兼顾性能、可维护性与生态
2. 滑动窗口 + 规则 + 控制台：核心数据流
...
## 我能马上用的 3 件事
1. [ ] 把 滑动窗口 + 规则 + 控制台 拆成 3 个步骤
2. [ ] 学 滑动窗口 + 规则 + 控制台 写一个 mini-sentinel
...
> sentinel 作为 流量控制 类项目，它的独特看点：

- **滑动窗口 + 规则 + 控制台** → 该方向在 流量控制 领域已被广泛验证，兼顾性能、可维护性与生态
- **滑动窗口 + 规则 + 控制台** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 sentinel = 计划书 + 框架图 + 滑动窗口 + 规则 + 控制台 + 跑起来 + 偷过来。
