...
> 字节 Coze 开源版，AI Agent 构建平台
> 来源：项目\coze-studio-main.zip
...
|---|---|
| 项目名 | coze-studio |
| 一句话定位 | 字节 Coze 开源版，AI Agent 构建平台 |
| 核心问题 | 解决「插件生态 + 工作流」领域的核心痛点：字节 Coze 开源版，AI Agent 构建平台 |
| 目标用户 | AI 应用开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际配置入口**：`- `common/_templates/component/package.json`
- `common/_templates/node-core/package.json`
- `common/_templates/rsbuild-web/package.json`
- `common/_templates/rspack-web/package.json`
- `common/_templates/solid-component/package.json``

**实际代码入口**：`backend/main.go`

**核心目录**（文件数最多）：`frontend/packages/components/bot-icons/src/assets/icons`, `frontend/packages/components/bot-semi/src/semi`, `docker/volumes/minio/default_icon/workflow_icon`, `frontend/packages/arch/idl/src/auto-generated/bytefaas_api/namespaces`, `frontend/packages/workflow/playground/src/hooks`, `frontend/packages/arch/bot-api/src`
...
| 总文件数 | 20491 |
| 主语言 | TypeScript |
| 涉及语言 | Go, JavaScript, Markdown, SQL, Shell, TypeScript, YAML |
| Star | 15k+ |
...
**[点状解析]**：LLM平台 项目的核心架构看点是 **插件生态 + 工作流**。
...
- 状态：已采纳
- 背景：解决「插件生态 + 工作流」领域的核心痛点：字节 Coze 开源版，AI Agent 构建平台
- 决策：采用 插件生态 + 工作流 作为核心架构思路
...
**前 5 个最大源码文件**：
```
1. `backend/api/model/passport/passport.go`
2. `backend/api/model/resource/resource.go`
3. `backend/api/model/playground/prompt_resource.go`
4. `backend/application/workflow/workflow.go`
5. `backend/api/handler/coze/workflow_service_test.go`
```
...
- **插件生态 + 工作流**：这是 coze-studio 的"灵魂"功能，必须精读
- **插件生态 + 工作流**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ✅ |
| Lint 配置 | 见 - `common/_templates/component/package.json`
- `common/_templates/node-core/package.json`
- `common/_templates/rsbuild-web/package.json`
- `common/_templates/rspack-web/package.json`
- `common/_templates/solid-component/package.json` |
...
**关键配置文件**：`- `common/_templates/component/package.json`
- `common/_templates/node-core/package.json`
- `common/_templates/rsbuild-web/package.json`
- `common/_templates/rspack-web/package.json`
- `common/_templates/solid-component/package.json``
...
```markdown
## 7 天复刻路径（以 coze-studio 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 backend/main.go → 理解启动流程
- D3: 读核心目录 `frontend/packages/components/bot-icons/src/assets/icons`, `frontend/packages/components/bot-semi/src/semi`, `docker/volumes/minio/default_icon/workflow_icon`, `frontend/packages/arch/idl/src/auto-generated/bytefaas_api/namespaces`, `frontend/packages/workflow/playground/src/hooks`, `frontend/packages/arch/bot-api/src` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-coze-studio（只保留核心）
- D6: 把 插件生态 + 工作流 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | coze-studio 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 插件生态 + 工作流：该方向在 LLM平台 领域已被广泛验证，兼顾性能、可维护性与生态
2. 插件生态 + 工作流：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. backend/main.go — 启动流程
2. backend/api/model/passport/passport.go — 核心实现
3. backend/api/model/resource/resource.go — 关键算法
4. backend/api/model/playground/prompt_resource.go — 性能优化
5. backend/application/workflow/workflow.go — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 插件生态 + 工作流 拆成 3 个步骤
2. [ ] 学 插件生态 + 工作流 写一个 mini-coze-studio
...
> coze-studio 作为 LLM平台 类项目，它的独特看点：

- **插件生态 + 工作流** → 该方向在 LLM平台 领域已被广泛验证，兼顾性能、可维护性与生态
- **插件生态 + 工作流** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 coze-studio = 计划书 + 框架图 + 插件生态 + 工作流 + 跑起来 + 偷过来。
