# kubernetes - 容器编排事实标准：声明式 API + Controller + Scheduler
...
```
pkg/kubelet/         # 节点代理
pkg/controller/      # controller-manager
pkg/scheduler/       # Filter + Score 调度
staging/src/k8s.io/  # api / apimachinery / client-go 子库
etcd                 # 强一致 KV 存储
```
...
**解决方案**：每个 controller 跑独立 goroutine；`Informer` watch 资源 + 事件入本地 cache；`workqueue` 去重保证 1 资源 1 处理；`reconcile(key)` 读期望 + 实际 + 调成；`RequeueAfter` 定时重试。
```go
...
// live (etcd 当前) + new (用户 apply) + lastApplied (注解) → 字段级 diff
```

## 必偷 3 件

1. **声明式 API + reconcile loop**：业务"声明期望 → 框架自动调成"；比命令式好测 10x；任何 IaC / 自动化运维通用。
2. **Controller + workqueue + Informer 3 件套**：watch + 缓存 + 去重 + 失败重试；事件驱动型业务黄金模式。
3. **CRD + Operator + controller-runtime**：让用户用 K8s 同等 API 自定义资源；平台中的平台——任何业务域可下沉。

## 必避 3 坑

1. **不要硬绑 Docker**——K8s 已抽 CRI / CNI / CSI 3 接口；硬绑 runtime 锁死升级路径。
2. **不要跨 namespace 直接访问 Service**——用 FQDN `service.namespace.svc.cluster.local`；短名只在同 ns 解析。
3. **不要用 `kubectl edit` 后再 `kubectl apply`**——会触发 3-way merge 冲突；`edit` 后 last-applied 同步更新。
