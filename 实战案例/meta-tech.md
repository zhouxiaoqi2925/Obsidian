...
|---|---|
| 项目名 | meta-tech |
| 一句话定位 | Meta/Facebook 大规模系统 + 数据库 + 基础设施 |
| 核心问题 | 解决「TAO + MyRocks + HHVM + Haystack」领域的核心痛点：Meta/Facebook 大规模系统 + 数据库 + 基础设施 |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。C++/Hack 项目常见布局：include/ + src/ + tests/
...
**[点状解析]**：技术博客 项目的核心架构看点是 **TAO + MyRocks + HHVM + Haystack**。
...
**核心架构看点**（TAO + MyRocks + HHVM + Haystack）：
- TAO + MyRocks + HHVM + Haystack
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 技术博客 方向**
- 状态：已采纳
- 背景：解决「TAO + MyRocks + HHVM + Haystack」领域的核心痛点：Meta/Facebook 大规模系统 + 数据库 + 基础设施
- 决策：采用 TAO + MyRocks + HHVM + Haystack 作为核心架构思路
...
- **TAO + MyRocks + HHVM + Haystack**：这是 meta-tech 的"灵魂"功能，必须精读
- **TAO + MyRocks + HHVM + Haystack**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **TAO + MyRocks + HHVM + Haystack**（meta-tech 的核心）
   - 实现思路：该方向在 技术博客 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：TAO + MyRocks + HHVM + Haystack
   - 自己项目：可借鉴到 开源 + 周边服务

2. **TAO + MyRocks + HHVM + Haystack**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 meta-tech 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-meta-tech（只保留核心）
- D6: 把 TAO + MyRocks + HHVM + Haystack 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | meta-tech 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. TAO + MyRocks + HHVM + Haystack：该方向在 技术博客 领域已被广泛验证，兼顾性能、可维护性与生态
2. TAO + MyRocks + HHVM + Haystack：核心数据流
...
## 1 个可复用模式
- TAO + MyRocks + HHVM + Haystack 实现方式

## 我能马上用的 3 件事
1. [ ] 把 TAO + MyRocks + HHVM + Haystack 拆成 3 个步骤
2. [ ] 学 TAO + MyRocks + HHVM + Haystack 写一个 mini-meta-tech
...
> meta-tech 作为 技术博客 类项目，它的独特看点：

- **TAO + MyRocks + HHVM + Haystack** → 该方向在 技术博客 领域已被广泛验证，兼顾性能、可维护性与生态
- **TAO + MyRocks + HHVM + Haystack** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 meta-tech = 计划书 + 框架图 + TAO + MyRocks + HHVM + Haystack + 跑起来 + 偷过来。
