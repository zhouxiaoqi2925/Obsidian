...
| 项目名 | jeelowcode |
| 一句话定位 | Java 企业级低代码平台 |
| 核心问题 | 解决「表单设计器 + 代码生成」领域的核心痛点：Java 企业级低代码平台 |
| 目标用户 | 企业 IT / 业务人员 |
| 商业模式 | 商业版授权 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际配置入口**：`- `jeelowcode-admin/pom.xml`
- `jeelowcode-core/pom.xml`
- `jeelowcode-dependencies/pom.xml`
- `jeelowcode-framework/jeelowcode-excel/pom.xml`
- `jeelowcode-framework/jeelowcode-exception/pom.xml``

**实际代码入口**：``

**核心目录**（文件数最多）：`jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/mapper`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/service`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/service/impl`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/entity`, `jeelowcode-core/src/main/java/com/jeelowcode/core/framework/mapper`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/controller`
...
**[点状解析]**：低代码 项目的核心架构看点是 **表单设计器 + 代码生成**。
...
**ADR-001: 为什么是 低代码 方向**
- 状态：已采纳
- 背景：解决「表单设计器 + 代码生成」领域的核心痛点：Java 企业级低代码平台
- 决策：采用 表单设计器 + 代码生成 作为核心架构思路
...
- **表单设计器 + 代码生成**：这是 jeelowcode 的"灵魂"功能，必须精读
- **表单设计器 + 代码生成**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ❌ |
| Lint 配置 | 见 - `jeelowcode-admin/pom.xml`
- `jeelowcode-core/pom.xml`
- `jeelowcode-dependencies/pom.xml`
- `jeelowcode-framework/jeelowcode-excel/pom.xml`
- `jeelowcode-framework/jeelowcode-exception/pom.xml` |
...
**关键配置文件**：`- `jeelowcode-admin/pom.xml`
- `jeelowcode-core/pom.xml`
- `jeelowcode-dependencies/pom.xml`
- `jeelowcode-framework/jeelowcode-excel/pom.xml`
- `jeelowcode-framework/jeelowcode-exception/pom.xml``
...
```markdown
## 7 天复刻路径（以 jeelowcode 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/mapper`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/service`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/service/impl`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/entity`, `jeelowcode-core/src/main/java/com/jeelowcode/core/framework/mapper`, `jeelowcode-service/jeelowcode-service-system-biz/src/main/java/com/jeelowcode/service/system/controller` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-jeelowcode（只保留核心）
- D6: 把 表单设计器 + 代码生成 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | jeelowcode 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 表单设计器 + 代码生成：该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
2. 表单设计器 + 代码生成：核心数据流
...
## 我能马上用的 3 件事
1. [ ] 把 表单设计器 + 代码生成 拆成 3 个步骤
2. [ ] 学 表单设计器 + 代码生成 写一个 mini-jeelowcode
...
> jeelowcode 作为 低代码 类项目，它的独特看点：

- **表单设计器 + 代码生成** → 该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
- **表单设计器 + 代码生成** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 jeelowcode = 计划书 + 框架图 + 表单设计器 + 代码生成 + 跑起来 + 偷过来。
