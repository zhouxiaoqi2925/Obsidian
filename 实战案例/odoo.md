...
|---|---|
| 项目名 | odoo |
| 一句话定位 | 开源企业应用套件（ERP/CRM/电商） |
| 核心问题 | 解决「ORM + 模块市场 + 工作流」领域的核心痛点：开源企业应用套件（ERP/CRM/电商） |
| 目标用户 | 企业 / 业务 |
...
**[点状解析]**：不读代码，先看"目录怎么长"。Python/JS 项目常见布局：package_name/ + tests/
...
**实际配置入口**：`- `addons/web/tooling/_package.json`
- `addons/point_of_sale/tests/test_pos_setup.py`
- `odoo/addons/test_lint/tests/_pylint_path_setup.py`
- `setup.py`
- `setup.cfg``

**实际代码入口**：`addons/auth_oauth/controllers/main.py`

**核心目录**（文件数最多）：`doc/cla/individual`, `doc/cla/corporate`, `odoo/addons/base/static/img/country_flags`, `addons/payment/static/img`, `addons/web/static/lib/pdfjs/web/cmaps`, `addons/website/views/snippets`
...
| 总文件数 | 55817 |
| 主语言 | Python |
| 涉及语言 | JavaScript, Markdown, Python, SQL, Shell, TypeScript |
| Star | 44k+ |
| License | GPL |
| Docker 支持 | ❌ |
| K8s 支持 | ❌ |
...
**[点状解析]**：ERP 项目的核心架构看点是 **ORM + 模块市场 + 工作流**。
...
**ADR-001: 为什么是 ERP 方向**
- 状态：已采纳
- 背景：解决「ORM + 模块市场 + 工作流」领域的核心痛点：开源企业应用套件（ERP/CRM/电商）
- 决策：采用 ORM + 模块市场 + 工作流 作为核心架构思路
...
**前 5 个最大源码文件**：
```
1. `addons/account/models/account_move_line.py`
2. `addons/mrp/models/mrp_production.py`
3. `addons/point_of_sale/tests/test_frontend.py`
4. `odoo/addons/base/models/ir_ui_view.py`
5. `addons/stock/tests/test_move2.py`
```

**入口文件**：`addons/auth_oauth/controllers/main.py`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：addons/auth_oauth/controllers/main.py
...
- **ORM + 模块市场 + 工作流**：这是 odoo 的"灵魂"功能，必须精读
...
| Docker | ❌ |
| K8s | ❌ |
| Lint 配置 | 见 - `addons/web/tooling/_package.json`
- `addons/point_of_sale/tests/test_pos_setup.py`
- `odoo/addons/test_lint/tests/_pylint_path_setup.py`
- `setup.py`
- `setup.cfg` |
...
**关键配置文件**：`- `addons/web/tooling/_package.json`
- `addons/point_of_sale/tests/test_pos_setup.py`
- `odoo/addons/test_lint/tests/_pylint_path_setup.py`
- `setup.py`
- `setup.cfg``
...
## 7 天复刻路径（以 odoo 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 addons/auth_oauth/controllers/main.py → 理解启动流程
- D3: 读核心目录 `doc/cla/individual`, `doc/cla/corporate`, `odoo/addons/base/static/img/country_flags`, `addons/payment/static/img`, `addons/web/static/lib/pdfjs/web/cmaps`, `addons/website/views/snippets` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-odoo（只保留核心）
- D6: 把 ORM + 模块市场 + 工作流 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | odoo 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. ORM + 模块市场 + 工作流：该方向在 ERP 领域已被广泛验证，兼顾性能、可维护性与生态
2. DocType 元数据驱动：模块市场 + 应用隔离
3. ORM + 工作流引擎：可直接借鉴到自己的项目

## 5 段必读代码
1. addons/auth_oauth/controllers/main.py — 启动流程
2. addons/account/models/account_move_line.py — 核心实现
3. addons/mrp/models/mrp_production.py — 关键算法
4. addons/point_of_sale/tests/test_frontend.py — 性能优化
5. odoo/addons/base/models/ir_ui_view.py — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 ORM + 模块市场 + 工作流 拆成 3 个步骤
2. [ ] 学 DocType 元数据驱动 写一个 mini-odoo
...
> odoo 作为 ERP 类项目，它的独特看点：

- **ORM + 模块市场 + 工作流** → 该方向在 ERP 领域已被广泛验证，兼顾性能、可维护性与生态
...
> 解析 odoo = 计划书 + 框架图 + ORM + 模块市场 + 工作流 + 跑起来 + 偷过来。
