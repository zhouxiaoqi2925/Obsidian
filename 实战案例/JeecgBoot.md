...
|---|---|
| 项目名 | JeecgBoot |
| 一句话定位 | 基于代码生成器的低代码开发平台 |
| 核心问题 | 解决「代码生成 + 表单引擎 + 工作流」领域的核心痛点：基于代码生成器的低代码开发平台 |
| 目标用户 | 企业 IT / 业务人员 |
| 商业模式 | 商业版授权 |
| 复刻难度 | ⭐⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**实际配置入口**：`- `jeecgboot-vue3/package.json`
- `jeecgboot-vue3/tests/server/package.json`
...
**实际代码入口**：`jeecgboot-vue3/electron/main.ts`

**核心目录**（文件数最多）：`jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/resources/static/generic/web/cmaps`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/resources/static/generic/web/images`, `jeecgboot-vue3/src/assets/images/wordtpl/images`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/java/org/jeecg/modules/system/service/impl`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/java/org/jeecg/modules/system/entity`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/java/org/jeecg/modules/system/mapper`
...
| 总文件数 | 5321 |
| 主语言 | Java |
| 涉及语言 | Java, JavaScript, Markdown, SQL, TypeScript, Vue, YAML |
| Star | 42k+ |
...
**[点状解析]**：低代码 项目的核心架构看点是 **代码生成 + 表单引擎 + 工作流**。
...
**ADR-001: 为什么是 低代码 方向**
- 状态：已采纳
- 背景：解决「代码生成 + 表单引擎 + 工作流」领域的核心痛点：基于代码生成器的低代码开发平台
- 决策：采用 代码生成 + 表单引擎 + 工作流 作为核心架构思路
- 理由：该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
...
**前 5 个最大源码文件**：
```
1. `jeecgboot-vue3/public/resource/js/iconfont.js`
2. `jeecgboot-vue3/src/components/onlinePreview/open-jssdk.es.js`
3. `jeecgboot-vue3/src/components/wordtpl/DocDesign.ts`
4. `jeecgboot-vue3/src/utils/index.ts`
5. `jeecgboot-vue3/src/utils/common/compUtils.ts`
```

**入口文件**：`jeecgboot-vue3/electron/main.ts`

### 5.2 单文件分析卡（入口示例）

```markdown
## 文件：jeecgboot-vue3/electron/main.ts
...
- **代码生成 + 表单引擎 + 工作流**：这是 JeecgBoot 的"灵魂"功能，必须精读
- **代码生成 + 表单引擎 + 工作流**：核心架构创新
- **核心数据流**：性能/可用性关键
...
| Docker | ✅ |
| K8s | ❌ |
| Lint 配置 | 见 - `jeecgboot-vue3/package.json`
- `jeecgboot-vue3/tests/server/package.json`
- `jeecg-boot/jeecg-boot-base-core/pom.xml`
- `jeecg-boot/jeecg-boot-module/jeecg-boot-module-airag/pom.xml`
- `jeecg-boot/jeecg-boot-module/jeecg-module-demo/pom.xml` |
...
**关键配置文件**：`- `jeecgboot-vue3/package.json`
- `jeecgboot-vue3/tests/server/package.json`
- `jeecg-boot/jeecg-boot-base-core/pom.xml`
- `jeecg-boot/jeecg-boot-module/jeecg-boot-module-airag/pom.xml`
- `jeecg-boot/jeecg-boot-module/jeecg-module-demo/pom.xml``
...
```markdown
1. **代码生成 + 表单引擎 + 工作流**（JeecgBoot 的核心）
   - 实现思路：该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：代码生成 + 表单引擎 + 工作流
   - 自己项目：可借鉴到 商业版授权

2. **代码生成 + 表单引擎 + 工作流**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
```markdown
## 7 天复刻路径（以 JeecgBoot 为例）
- D1: 跑起来 → 混个脸熟
- D2: 读 jeecgboot-vue3/electron/main.ts → 理解启动流程
- D3: 读核心目录 `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/resources/static/generic/web/cmaps`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/resources/static/generic/web/images`, `jeecgboot-vue3/src/assets/images/wordtpl/images`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/java/org/jeecg/modules/system/service/impl`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/java/org/jeecg/modules/system/entity`, `jeecg-boot/jeecg-module-system/jeecg-system-biz/src/main/java/org/jeecg/modules/system/mapper` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-JeecgBoot（只保留核心）
- D6: 把 代码生成 + 表单引擎 + 工作流 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | JeecgBoot 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 代码生成 + 表单引擎 + 工作流：该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
2. 代码生成 + 表单引擎 + 工作流：核心数据流
3. 性能/可用性关键点：可直接借鉴到自己的项目

## 5 段必读代码
1. jeecgboot-vue3/electron/main.ts — 启动流程
2. jeecgboot-vue3/public/resource/js/iconfont.js — 核心实现
3. jeecgboot-vue3/src/components/onlinePreview/open-jssdk.es.js — 关键算法
4. jeecgboot-vue3/src/components/wordtpl/DocDesign.ts — 性能优化
5. jeecgboot-vue3/src/utils/index.ts — 边界处理
...
## 我能马上用的 3 件事
1. [ ] 把 代码生成 + 表单引擎 + 工作流 拆成 3 个步骤
2. [ ] 学 代码生成 + 表单引擎 + 工作流 写一个 mini-JeecgBoot
3. [ ] 把 核心数据流 用到自己的 商业版授权
```
...
> JeecgBoot 作为 低代码 类项目，它的独特看点：

- **代码生成 + 表单引擎 + 工作流** → 该方向在 低代码 领域已被广泛验证，兼顾性能、可维护性与生态
- **代码生成 + 表单引擎 + 工作流** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践

**与同类的对比**：
vs OutSystems / Mendix：开源 + 二次开发友好
...
> 解析 JeecgBoot = 计划书 + 框架图 + 代码生成 + 表单引擎 + 工作流 + 跑起来 + 偷过来。
