...
> Grafana 出品的「Prometheus for logs」
> 来源：（无本地 zip，依据公开知识）
...
|---|---|
| 项目名 | loki |
| 一句话定位 | Grafana 出品的「Prometheus for logs」 |
| 核心问题 | 解决「标签索引 + 廉价存储 + LogQL」领域的核心痛点：Grafana 出品的「Prometheus for logs」 |
| 目标用户 | 开发者 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**[点状解析]**：日志聚合 项目的核心架构看点是 **标签索引 + 廉价存储 + LogQL**。
...
**核心架构看点**（标签索引 + 廉价存储 + LogQL）：
- 标签索引 + 廉价存储 + LogQL
- 核心数据流
- 性能/可用性关键点

**ADR-001: 为什么是 日志聚合 方向**
- 状态：已采纳
- 背景：解决「标签索引 + 廉价存储 + LogQL」领域的核心痛点：Grafana 出品的「Prometheus for logs」
- 决策：采用 标签索引 + 廉价存储 + LogQL 作为核心架构思路
- 理由：该方向在 日志聚合 领域已被广泛验证，兼顾性能、可维护性与生态
...
- **标签索引 + 廉价存储 + LogQL**：这是 loki 的"灵魂"功能，必须精读
- **标签索引 + 廉价存储 + LogQL**：核心架构创新
- **核心数据流**：性能/可用性关键
...
```markdown
1. **标签索引 + 廉价存储 + LogQL**（loki 的核心）
   - 实现思路：该方向在 日志聚合 领域已被广泛验证，兼顾性能、可维护性与生态
   - 应用场景：标签索引 + 廉价存储 + LogQL
   - 自己项目：可借鉴到 开源 + 周边服务

2. **标签索引 + 廉价存储 + LogQL**（架构设计）
   - 解耦了什么/怎么解耦
   - 借鉴到自己的分层架构
...
- D3: 读核心目录 未识别 → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-loki（只保留核心）
- D6: 把 标签索引 + 廉价存储 + LogQL 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | loki 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 标签索引 + 廉价存储 + LogQL：该方向在 日志聚合 领域已被广泛验证，兼顾性能、可维护性与生态
2. 标签索引 + 廉价存储 + LogQL：核心数据流
...
## 1 个可复用模式
- 标签索引 + 廉价存储 + LogQL 实现方式

## 我能马上用的 3 件事
1. [ ] 把 标签索引 + 廉价存储 + LogQL 拆成 3 个步骤
2. [ ] 学 标签索引 + 廉价存储 + LogQL 写一个 mini-loki
...
> loki 作为 日志聚合 类项目，它的独特看点：

- **标签索引 + 廉价存储 + LogQL** → 该方向在 日志聚合 领域已被广泛验证，兼顾性能、可维护性与生态
- **标签索引 + 廉价存储 + LogQL** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 loki = 计划书 + 框架图 + 标签索引 + 廉价存储 + LogQL + 跑起来 + 偷过来。
