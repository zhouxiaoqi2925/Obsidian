...
> transformers 是 NLP/CV/多模态的事实标准——200+ 模型架构统一在 `AutoModel` 工厂下，`PreTrainedModel` 契约把 forward/save/load/generate 抽象成统一接口，`Trainer` 封装 PyTorch 训练循环。配套 safetensors 零拷贝、PEFT/LoRA 参数高效微调、accelerate 分布式、optimum ONNX 部署——一个 Python 库覆盖整个 ML 生命周期。
...
**解决方案**：`AutoModel` / `AutoTokenizer` / `AutoConfig` 等 Auto 类是工厂，配合 `MODEL_MAPPING` / `MODEL_FOR_SEQUENCE_CLASSIFICATION_MAPPING` 等注册表，按 `config.architectures` 字段自动选具体模型类。`from_pretrained()` 一行加载。
...
**最佳实践**：训练/推理用同一种 tokenizer；多句用 `tokenizer(text_pair=...)`；长文本用 `return_overflowing_tokens=True` 滑窗；`tokenizer.save_pretrained()` 持久化。
...
**解决方案**：`pipeline(task="text-generation", model="gpt2")` 一键构造推理管线，内部串联 `Tokenizer` + `Model` + `Postprocessor`。支持 30+ 任务（text-generation / text-classification / ner / qa / summarization / feature-extraction / zero-shot-classification）。
...
**解决方案**：PEFT（Parameter-Efficient Fine-Tuning）库实现 LoRA / QLoRA / Adapter / Prefix-Tuning。LoRA 把权重变化 `ΔW` 拆成低秩 `BA`（r=8-64），只训 BA，参数 1% 以下。QLoRA = 4-bit 基础模型 + LoRA。
...
### 模式 18 · 缓存与批处理优化（vLLM / PagedAttention）
...
**3 核心洞察**：
1. AutoModel 工厂 + 注册表 = 200+ 模型统一接口的范本
2. PreTrainedModel 契约 + forward 抽象 = 任何模型都能复用训练/部署生态
3. PEFT/LoRA + bitsandbytes = 单卡 4-bit 训 70B 的工业现实

**1 反模式**：训练时用 pickle 权重（`pytorch_model.bin`）——既慢又不安全，safetensors 全面取代。

**3 立刻能用**：
1. `AutoModel.from_pretrained("bert-base-uncased")` 一行加载任何模型
2. `LoraConfig(r=16, target_modules=["q_proj","v_proj"])` + `get_peft_model()` 1% 参数微调
3. `pipeline("text-generation", model="gpt2")` 30+ 任务零代码推理
