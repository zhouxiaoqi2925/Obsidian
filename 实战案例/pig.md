...
> 基于 Spring Cloud 的企业级开发平台
> 来源：项目\pig-master.zip
...
|---|---|
| 项目名 | pig |
| 一句话定位 | 基于 Spring Cloud 的企业级开发平台 |
| 核心问题 | 解决「微服务 + 代码生成 + 多租户」领域的核心痛点：基于 Spring Cloud 的企业级开发平台 |
| 目标用户 | Java 工程师 |
| 商业模式 | 开源 + 周边服务 |
| 复刻难度 | ⭐⭐⭐ |
| 当前状态 | 活跃 |
...
**核心目录**（文件数最多）：`pig-upms/pig-upms-api/src/main/java/com/pig4cloud/pig/admin/api/entity`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/controller`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/mapper`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/service`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/service/impl`, `pig-common/pig-common-xss/src/main/java/com/pig4cloud/pig/common/xss/core`
...
**[点状解析]**：快速开发 项目的核心架构看点是 **微服务 + 代码生成 + 多租户**。
...
- 状态：已采纳
- 背景：解决「微服务 + 代码生成 + 多租户」领域的核心痛点：基于 Spring Cloud 的企业级开发平台
- 决策：采用 微服务 + 代码生成 + 多租户 作为核心架构思路
...
- **微服务 + 代码生成 + 多租户**：这是 pig 的"灵魂"功能，必须精读
- **微服务 + 代码生成 + 多租户**：核心架构创新
- **核心数据流**：性能/可用性关键
...
- D1: 跑起来 → 混个脸熟
- D2: 读  → 理解启动流程
- D3: 读核心目录 `pig-upms/pig-upms-api/src/main/java/com/pig4cloud/pig/admin/api/entity`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/controller`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/mapper`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/service`, `pig-upms/pig-upms-biz/src/main/java/com/pig4cloud/pig/admin/service/impl`, `pig-common/pig-common-xss/src/main/java/com/pig4cloud/pig/common/xss/core` → 理解主流程
- D4: 跑测试 + 改一处 → 理解可扩展点
- D5: 自己写个 200 行的 mini-pig（只保留核心）
- D6: 把 微服务 + 代码生成 + 多租户 用到自己的项目
- D7: 写一篇博客把 5 天串起来
```

### 12.4 项目打分卡

| 维度 | 1 分 | 3 分 | 5 分 | pig 自评 |
|---|---|---|---|---|
| 代码质量 | 凑合 | 工业级 | 教科书 | ⭐⭐⭐⭐ |
...
## 3 个核心洞察
1. 微服务 + 代码生成 + 多租户：该方向在 快速开发 领域已被广泛验证，兼顾性能、可维护性与生态
2. 微服务 + 代码生成 + 多租户：核心数据流
...
## 我能马上用的 3 件事
1. [ ] 把 微服务 + 代码生成 + 多租户 拆成 3 个步骤
2. [ ] 学 微服务 + 代码生成 + 多租户 写一个 mini-pig
...
> pig 作为 快速开发 类项目，它的独特看点：

- **微服务 + 代码生成 + 多租户** → 该方向在 快速开发 领域已被广泛验证，兼顾性能、可维护性与生态
- **微服务 + 代码生成 + 多租户** → 核心数据流
- **性能/可用性关键点** → 可借鉴的工程实践
...
> 解析 pig = 计划书 + 框架图 + 微服务 + 代码生成 + 多租户 + 跑起来 + 偷过来。
