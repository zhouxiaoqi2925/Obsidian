...
> underscore 是 jQuery 之父 Jeremy Ashkenas 2009 年打造的 JS 工具带，比 ES5 早 6 年补齐 200+ 函数。`mixin` 让静态/链式双形态共存，`iteratee` 把 4 种 callback 形态归一，`trampoline` 解决深递归爆栈。17 年仍在维护——CVE 修复 + 老分支 LTS 兼容 + 主分支跟进现代 JS，"长生命周期库"的工程范本。
...
**解决方案**：`cb(value, context, argCount)` 内部把 4 形态归一为函数——函数 → 原样返回；字符串 `_.property("name")` → `(obj) => obj.name`；对象 `{active: true}` → `(obj) => obj.active === true`；null/未传 → `_.identity(x) => x`。
...
**解决方案**：Underscore 用 Makefile（不用 webpack）打包 5 个版本——`underscore.js` UMD（浏览器 + Node）/ `underscore-esm.js` ES Module / `underscore-node.cjs` CJS / `underscore-node.mjs` ESM / `underscore-amd.js` AMD。
...
**解决方案**：搜索框：`_.debounce(search, 300)`；滚动：`_.throttle(handleScroll, 100)`；提交按钮：`_.once(submit, 1000)` 限流；resize：`_.debounce(handleResize, 250)`。
...
**解决方案**：Underscore 模式——CVE 修复在源码注释内嵌原因（CVE-2021-23358 等）；老分支 1.13.x 仍维护（Node 0.10 兼容）；主分支支持现代 Node（`_.template` 升级）；测试覆盖 `tests/` 数百用例；`contrib/` 收集社区扩展。
...
**3 核心洞察**：
1. mixin = 静态/链式双形态共存的标准做法
2. iteratee 4 形态归一 = "callable 或 string 或 object 或 null"通用契约
3. trampoline = 无限递归不爆栈的函数式解法

**1 反模式**：trampoline 用于浅递归（开销大）——只在深度 > 100 的递归场景用。

**3 立刻能用**：
1. `_(arr).chain().map(...).filter(...).value()` 链式管道
2. `_.mixin({ myFn: ... })` 扩展业务方法，自动支持链式
3. `_.debounce(fn, 300)` + `_.throttle(fn, 100)` 高频事件标配
