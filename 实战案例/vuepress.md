# vuepress - Vue 驱动的静态站点生成器与 markdown-it 管线 + 多页 App 容器典范
...
**解决方案**：VuePress 用 `markdown-it` 作核心，链式 `use(plugin)` 加载扩展（`markdown-it-anchor`/`markdown-it-toc`/`markdown-it-container`）。`markdown.lineNumbers` 在 build 阶段注入 line numbers。
