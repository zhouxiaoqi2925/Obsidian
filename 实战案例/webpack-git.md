...
**适用场景**: 编译原理 / 构建工具开发 / AST 分析 / bundler 源码阅读

> webpack 5 源码层把"模块图谱、并发队列、深度 tree-shaking、CSS 抽离"做成 4 个独立子系统——ModuleGraph 抽离让插件 O(1) 反查 dep→module，AsyncQueue 把构建过程建模为多阶段并发队列，InnerGraph 让"嵌套符号引用"也参与 unused 判定，CssModule + processAssets 让 CSS 像 JS 一样走完整编译流程。理解这 4 个子系统就读懂 webpack 5 源码 70%。
...
**最佳实践**：用 `moduleGraph` API 而非 `compilation.modules`；用 `getModule` 由 dep 反查 module；用 `getDependencies` 替代 `module.dependencies`；写自定义 plugin 用 ModuleGraph API；v4 升级 v5 注意 API 变化。

### 模式 2 · AsyncQueue 多阶段并发队列

**问题场景**：构建过程是阶段性的（factorize → build → seal → emit），各阶段用数组 + forEach 串行——慢，且难并发。

**解决方案**：`AsyncQueue` 是 Webpack 5 的并发队列：`enqueue` 入队、`enqueueProvider` 显式依赖、`start` 启动 worker 池。`factorizeQueue` / `buildQueue` / `processDependenciesQueue` 都是 AsyncQueue 实例。`ParallelismPlugin` 限制并发数。
...
**最佳实践**：理解 AsyncQueue 调度；写自定义 stage 用 `enqueue`；`ParallelismPlugin` 配 `cpu - 1`；异步构建用 `start` / `stop`；监控队列长度；不乱调并发（OOM）。

### 模式 3 · ChunkGraph 与 Chunk 模块映射

**问题场景**：Webpack 4 的 Chunk 关系散在 `Chunk` 对象上，跨 Chunk 找模块困难。

**解决方案**：Webpack 5 抽离 `ChunkGraph`：每个 Chunk 关联 `ChunkGraph` 节点，记录 `chunkModules` / `chunkRuntimeModules` / `runtimeRequirements`。`chunkGraph.getModuleChunks(module)` 反查 Module 在哪些 Chunk。`chunkGraph.getChunkRuntimeRequirements(chunk)` 查 Chunk 运行时依赖。
...
**最佳实践**：用 `chunkGraph` API 替代 `chunk.modules`；`getModuleChunks` 反查；`runtimeRequirements` Set 配 chunk 运行时所需；写自定义 chunk 优化插件用 ChunkGraph；理解 chunk graph vs module graph 区别。

### 模式 4 · Factorize / Add / Build / Seal / Emit 五阶段

**问题场景**：Webpack 4 用回调串联（`buildModule` / `seal`）——阶段边界模糊，插件难定位。

**解决方案**：Webpack 5 显式分 5 阶段：① **Factorize** 从 entry 递归解析 dependency；② **Add** 注册 module 到 moduleGraph；③ **Build** loader 转换 + parser 解析依赖；④ **Seal** 优化（tree-shake / splitChunks / hashing）；⑤ **Emit** 写入 dist。每阶段对应钩子 + AsyncQueue 驱动。
...
**解决方案**：Webpack 5 拆 `Parser`（核心 + 钩子）+ `ParserPlugin`（语言扩展）。`parser.hooks.evaluate` 解析表达式（用于 tree-shake 判断）；`parser.hooks.statement` 解析语句；底层用 `acorn` 解析 JS AST。
...
**最佳实践**：用 `Parser` API 替代裸 `acorn`；`evaluate` 用于 dead code 分析；写自定义 import 解析用 `hooks.import`；`parser` 是单 Module 复用；tree-shake 依赖 evaluate 钩子；用 `swc` / `esbuild` 替代 acorn 加速。
...
**解决方案**：Webpack 5 引入 `InnerGraph`：分析模块内符号引用关系，标记 unused。`getInnerGraph(module)` 返回 `TopLevelDeclarations`。`FlagDependencyExportsPlugin` / `FlagUsageExportsPlugin` 配 InnerGraph 做传播标记。
...
**解决方案**：`MiniCssExtractPlugin` 走 Webpack 5 的 `processAssets` 钩子：① CSS loader 把 CSS 编译为 Module；② `CssModule` 类是 `Module` 子类；③ `CssExtractPlugin.loader` 收集 CSS 到 `module._css`；④ `processAssets` 阶段生成 `*.css` 文件；⑤ HTML 注入 `<link>`。
...
**最佳实践**：用 `MiniCssExtractPlugin` 抽 CSS；用 `css-loader` / `postcss-loader` 处理 CSS；用 `postcss` 配 autoprefixer；HMR 配 `style-loader`（dev）；生产配 `MiniCssExtractPlugin`（prod）。

### 模式 8 · runtime 与 runtimeRequirements

**问题场景**：Webpack 4 把 runtime 代码生成在每个 Chunk 中——重复且大。

**解决方案**：Webpack 5 显式 `runtime` + `runtimeRequirements`：① `runtimeRequirements: Set<string>` 标记 Chunk 需要的 runtime 函数；② `compilation.addRuntimeModule(chunk, runtimeModule)` 注入 runtime；③ `RuntimeGlobals` 枚举所有 runtime 符号；④ 优化：去重 + 按需注入。
...
**解决方案**：`stats.toJson()` 返回结构化统计：`modules` / `chunks` / `assets` / `errors` / `warnings`。`webpack-bundle-analyzer` 解析 `stats.json` 可视化。`stats.preset: 'minimal'` 简化输出。`stats.json` 可上传到 CI 做历史对比。
...
**解决方案**：Webpack 5 强化 `WebpackError` / `Diagnostic`：① `compilation.errors.push(new WebpackError('xxx'))` 抛错；② `Diagnostic` 是结构化错误（含定位）；③ 插件用 `getResolve()` 取 resolver 错误；④ `ModuleNotFoundError` / `ModuleParseError` 等内置错误类。
...
**解决方案**：Webpack 5 引入 `LazyCompilation` 插件：① `import()` 走 HTTP 触发；② `entry` 标记为 lazy；③ 服务端用 `webpack-dev-server` 配 `lazyCompilation`；④ 首次访问触发编译。
...
**解决方案**：Module Federation 由 4 个核心概念组成：① **Host** 消费方，声明 `remotes`；② **Remote** 提供方，声明 `exposes`；③ **Shared** 共享依赖（React / Vue）；④ **Federation Runtime** 运行时加载 remote 模块。Webpack 5 原生 `ModuleFederationPlugin` 实现。
...
**解决方案**：`experiments` 配置开启：① `outputModule: true` 输出 ESM；② `asyncWebAssembly: true` 异步 WASM；③ `topLevelAwait: true` 顶层 await；④ `layers: true` Layer 概念；⑤ `buildHttp: true` HTTP URL 作为 entry。
...
**解决方案**：Webpack 5 内置 `asset modules`：① `asset/resource` 输出文件（原 `file-loader`）；② `asset/inline` 内嵌 base64（原 `url-loader`）；③ `asset/source` 导出源码（原 `raw-loader`）；④ `asset` 自动选择（< 8KB inline）。CSS 用 `type: 'css'` 配 `experiments.css`。
...
**解决方案**：① `node --inspect-brk ./node_modules/.bin/webpack` 启用 Chrome DevTools；② `webpack --profile --json` 输出 profile；③ `speed-measure-webpack-plugin` 插件级耗时；④ Webpack 内置 `compilation.hooks.buildModule` 计时；⑤ 火焰图 `flamebearer`。
...
**解决方案**：`mode: 'production'` 启用压缩；`optimization.minimize: true`；`splitChunks` 拆 vendor；`runtimeChunk: 'single'`；`moduleIds: 'deterministic'`；`cache: { type: 'filesystem' }` 持久化；`thread-loader` 配多核；`image-webpack-loader` 压图；`purgecss-webpack-plugin` 删未用 CSS；`terser-webpack-plugin` parallel。
...
**解决方案**：常见错误类型：① **Module not found** resolve 配置 / `extensions` / `alias`；② **Loader not found** 没装 / 路径错；③ **Parse error** parser 配置 / Babel；④ **Module parse failed** loader 链漏；⑤ **Plugin tap error** Hook 名错 / 阶段错；⑥ **Out of memory** 单进程 / 减少并发。
...
**问题场景**：Webpack 会被 Vite / Rspack / Turbopack 取代吗？——Webpack 6 计划。

**解决方案**：① **Webpack 5** 当前稳定；② **Webpack 6** 计划中（Rust 部分 + 优化）；③ **Rspack** Rust 写 Webpack 兼容（10x 快）；④ **Turbopack** Vercel 的 Rust bundler；⑤ **Vite** 原生 ESM + Rollup；⑥ **Module Federation** 独立标准化。
...
**3 核心洞察**：
1. `ModuleGraph` 抽离 = 插件 O(1) 反查 dep → module，告别 v4 O(N) 遍历
2. `AsyncQueue` 多阶段队列 = 构建过程可并发、可监控、可插拔
3. `InnerGraph` 深度 tree-shake = 让"嵌套符号引用"也参与 unused 判定

**1 反模式**：在 `compilation.seal` 之后还调 `moduleGraph.setModule()`——seal 阶段后 graph 已冻结，再改会破坏 chunk 优化。

**3 立刻能用**：
1. `compilation.moduleGraph.getModule(dep)` 由 dep 反查 module
2. `compilation.hooks.processAssets.tap({ stage: PROCESS_ASSETS_STAGE_REPORT }, ...)` 处理产物
3. `module.rules: [{ test: /\.png$/, type: 'asset' }]` 走内置 asset module
