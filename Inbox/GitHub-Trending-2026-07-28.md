...
- **语言**: Rust | **Star**: 23024 | **Fork**: 4350
- **更新**: 2026-07-28 | **License**: Apache-2.0

#### 前面介绍

- Grok Build 是 SpaceXAI 开发的基于终端的 AI 编程代理工具。它作为一个全屏 TUI（终端用户界面）运行，具备理解代码库、编辑文件、执行 Shell 命令、搜索网络以及管理长期任务的能力。支持交互式使用、无头模式（用于脚本/CI）以及通过 Agent Client Protocol (ACP) 嵌入到编辑器中。该项目使用 Rust 编写。
...
- 项目采用 Rust 编写的 Monorepo 结构，核心逻辑分为多个 Crates。
- 主要组件包括：xai-grok-pager（负责 TUI 界面渲染、滚动、提示和模态框）、xai-grok-shell（Agent 运行时及入口点）、xai-grok-tools（工具实现，如终端、文件编辑、搜索等）以及 xai-grok-workspace（管理宿主机文件系统、版本控制、执行和检查点）。
- 通过 Agent Client Protocol (ACP) 实现与外部编辑器或客户端的通信。
- 支持 MCP (Model Context Protocol) 服务器集成，用于扩展 AI 的上下文感知能力。
- 构建系统依赖 DotSlash 来管理 hermetic 工具（如 bin/protoc）的下载和运行。
- 底层使用 Ratatui 库进行终端渲染，支持自定义主题和样式。
- 包含沙箱机制以确保安全执行代码。
- 支持插件系统，允许通过 Skills、Hooks 和 Plugins 进行功能扩展。

#### 运行方式

- 安装预编译二进制文件：在 macOS/Linux 上运行 curl -fsSL https://x.ai/cli/install.sh | bash，在 Windows 上运行 irm https://x.ai/cli/install.ps1 | iex。
- 从源码构建：需要安装 Rust 工具链（由 rust-toolchain.toml 指定版本）和 DotSlash 工具。
- 构建命令：使用 cargo run -p xai-grok-pager-bin 构建并启动 TUI，或使用 cargo build -p xai-grok-pager-bin --release 进行发布构建。
- 首次启动时，程序会打开浏览器进行身份验证。
- 构建环境支持 macOS 和 Linux，Windows 构建为尽力而为。
- 项目根目录的 Cargo.toml 是自动生成的，建议直接编辑各个 Crate 的 Cargo.toml 文件。

#### 项目亮点

- 全屏交互式 TUI：提供类似 IDE 的沉浸式终端体验，支持滚动回放、键盘快捷键和斜杠命令。
- 强大的代码库理解：能够深入分析代码结构，理解上下文并执行编辑。
- 多模式运行：支持交互式、无头（Headless）脚本模式和编辑器嵌入模式。
- Agent Client Protocol (ACP)：定义了与编辑器集成的标准协议。
- MCP 集成：原生支持 Model Context Protocol，便于连接各种外部数据源。
- 安全沙箱：内置沙箱机制以隔离执行环境。

#### 代码解析

- 项目结构清晰，分为 crates/codegen 和 crates/common 等目录，遵循 Rust Workspace 最佳实践。
- 核心入口点位于 crates/codegen/ptyctl-cli，这是一个用于无头 PTY（伪终端）控制的 CLI 工具，允许在后台运行 Shell 命令并与 TUI 交互。
- Agent 生命周期管理位于 xai-agent-lifecycle crate，负责处理会话状态和轮次输入。
- ACP (Agent Client Protocol) 实现在 xai-acp-lib crate 中，定义了消息格式、通道和标准化逻辑。
- xai-grok-tools 提供了具体的工具实现，如文件编辑、搜索等，这些工具被 Agent 调用以执行任务。
- xai-grok-workspace 负责与宿主机文件系统交互，并集成了 git 功能用于版本控制。
- 构建系统包含 xai-proto-build，用于处理 Protobuf 代码生成。
- 代码中大量使用了 async/await 和 Tokio 运行时，体现了异步编程的范式。
...
- **语言**: JavaScript | **Star**: 12459 | **Fork**: 1253
- **更新**: 2026-07-28 | **License**: 未知
...
- **语言**: Python | **Star**: 8844 | **Fork**: 1155
- **更新**: 2026-07-28 | **License**: MIT
...
- **语言**: Rust | **Star**: 7616 | **Fork**: 12
- **更新**: 2026-07-28 | **License**: 未知

#### 前面介绍

- AOS Community Edition 是一个开源的智能体操作系统，旨在为智能体提供一个可检查、可组合的环境。它不仅管理智能体运行时，还拥有完整的产品界面，包括 CLI 工具、HTTP API、官方胶囊、模型体验以及 Unicity Audit。该系统通过 'Capsule'（胶囊）作为通用的用户空间构建块，允许用户将它们组合成各种系统。
...
- 工作区布局：包含 crates（产品 CLI、API、控制客户端）、capsules（官方生产胶囊）、distros（发行版清单）和 docs（文档）。
- 命令边界：AOS 拥有 init、status、mcp 等产品根命令，其他运行时命令直接挂载在 aos CLI 下，不存在嵌套命名空间。
- MCP 边缘：aos mcp serve 是 Codex、Claude 和 Grok 共享的产品边缘，支持本地决策表面（AppKit、Windows 对话框、Pinentry）。
- 构建基石：Capsule 是通用用户空间构建块，Forge 是 OS 构建工具，Meta Harness 是教授智能体构建受控系统的技能。
- 进程模型：aos daemon foreground 在 Unix 上替换进程以保持信号所有权，在其他主机上等待守护进程。
- 安全与验证：发布前必须通过机器可读的运行时兼容性检查和升级/自愈门控，确保候选版本能成功启动。
- CLI 代理：capsule-cli 是显示服务器，通过 Unix 域 socket 连接内核 IPC 事件总线和 TUI 前端，并维护严格的入站允许列表。

#### 运行方式

- 安装方式：使用官方安装脚本 curl --proto '=https' --tlsv1.2 -fsSL https://aos.unicity.ai/install.sh | sh。
- 初始化：运行 aos init（支持 --offline 离线模式）从本地资产进行配置。
- 工作目录：所有产品版本化的胶囊资产安装在产品拥有的 ~/.aos 根目录下。
- 版本管理：Homebrew 安装可通过 aos update 更新，直接安装默认解析 signed stable 通道，也可选择 dev、nightly 或特定版本。
- 多主体：使用 aos --principal operator init --target-principal alice 为不同主体进行配置，保持操作员与目标环境分离。
- 构建环境：项目使用 Rust Workspace，依赖 astrid-sdk、axum、tokio 等核心库，MSRV 为 Rust 1.94。

#### 项目亮点

- 可检查与可组合：提供透明的环境，允许智能体检查系统并组合胶囊来扩展能力。
- 严格的命令所有权：通过发布验证确保只有明确的继承或拥有决策才能将新命令引入产品发布。
- 安全交互：MCP 协议集成支持本地决策表面，仅接受布尔值或固定枚举，拒绝任意字符串或密码字段。
- 自愈与升级：发布包含 Sigstore 签名、GitHub 构建证明和 runtime-compatibility.toml，确保升级过程安全且可自愈。
- Meta Harness 技能：内置 meta-harness 技能，教导智能体如何构建受控的元 Harness，将世界视为可改进的用户空间。
- CLI 代理机制：capsule-cli 采用多客户端接受循环，支持最多 8 个并发连接，并严格过滤 IPC 主题以防止未授权访问。

#### 代码解析

- Cargo.toml 配置：定义了包含 23 个成员的 Rust Workspace，包括核心 SDK 依赖和发布配置（opt-level=z, LTO, strip）。
- capsule-agents：负责将 AGENTS.md 中的项目指令注入到系统提示词中，依赖 astrid-sdk 和 serde。
- capsule-cli：作为 CLI 代理，绑定 Unix socket，订阅 TUI 相关 IPC 主题，并维护活跃连接列表以防止槽位耗尽。
- capsule-context-engine：提供上下文策略引擎，支持策略定义和测试。
- capsule-forge：包含构建指南和权限指南，是构建和验证最小权限胶囊的工具。
- 依赖版本锁定：所有 astrid 相关依赖（sdk, core, types, uplink）均被锁定为特定版本（0.7.1, 0.10.4），确保稳定性。
- 许可证策略：项目采用 MIT 或 Apache-2.0 双重许可，capsule-cli 特别注明了 MSRV 为 1.94。
- 构建产物优化：release 配置启用了代码优化、链接时优化和符号剥离，以减小二进制文件体积。
...
| Classic Knife · Fade (Minimal Wear) | CS2 weapon | [Live](https://img2threejs.github.io/img2thr
```
...
- **语言**: Rust
- **Star**: 23024 | **Fork**: 4350 | **更新**: 2026-07-28
...
- **语言**: C
- **Star**: 20199 | **Fork**: 2024 | **更新**: 2026-07-28

### 3. Fei-Away/Codex-Dream-Skin
- **仓库**: [Fei-Away/Codex-Dream-Skin](https://github.com/Fei-Away/Codex-Dream-Skin)
- **描述**: Codex Dream Skin
- **语言**: JavaScript
- **Star**: 12458 | **Fork**: 1253 | **更新**: 2026-07-28

### 4. andrewyng/openworker
- **仓库**: [andrewyng/openworker](https://github.com/andrewyng/openworker)
- **语言**: Python
- **Star**: 8841 | **Fork**: 1155 | **更新**: 2026-07-28

### 5. unicity-aos/aos-ce
- **仓库**: [unicity-aos/aos-ce](https://github.com/unicity-aos/aos-ce)
- **描述**: AOS Community Edition: the open agent operating system.
- **语言**: Rust
- **Star**: 7616 | **Fork**: 12 | **更新**: 2026-07-28
...
- **语言**: Python
- **Star**: 6977 | **Fork**: 535 | **更新**: 2026-07-28

### 7. x4gKing/X4G
- **仓库**: [x4gKing/X4G](https://github.com/x4gKing/X4G)
- **语言**: Python
- **Star**: 6892 | **Fork**: 12617 | **更新**: 2026-07-28
...
- **语言**: JavaScript
- **Star**: 5515 | **Fork**: 633 | **更新**: 2026-07-28
...
- **语言**: TypeScript
- **Star**: 3402 | **Fork**: 269 | **更新**: 2026-07-28
...
- **语言**: Python
- **Star**: 3279 | **Fork**: 538 | **更新**: 2026-07-28
...
- **语言**: Python
- **Star**: 2988 | **Fork**: 233 | **更新**: 2026-07-28
...
- **语言**: TypeScript
- **Star**: 2891 | **Fork**: 403 | **更新**: 2026-07-28
...
- **Star**: 2656 | **Fork**: 165 | **更新**: 2026-07-28
