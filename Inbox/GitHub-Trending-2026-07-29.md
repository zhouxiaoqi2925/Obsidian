...
- **语言**: Rust | **Star**: 23257 | **Fork**: 4404
- **更新**: 2026-07-29 | **License**: Apache-2.0

#### 前面介绍

- Grok Build 是 SpaceXAI 开发的终端界面 AI 编程代理工具。它是一个全屏 TUI 应用，能够理解代码库、编辑文件、执行 Shell 命令、搜索网络并管理长期任务。它支持交互式模式、无头模式（用于脚本/CI）以及通过 Agent Client Protocol (ACP) 嵌入到编辑器中。该项目使用 Rust 编写，包含完整的代理运行时、TUI 渲染引擎、工具实现和沙箱环境。
...
- 项目采用 Rust Workspace 多包管理结构，主要分为代码生成、核心运行时、TUI 渲染和工具实现四大类。
- xai-grok-pager-bin：组合根包，负责构建最终的可执行文件。
- xai-grok-pager：TUI 层，负责滚动回显、提示输入、模态框渲染和界面交互。
- xai-grok-shell：代理运行时，包含领导模式、标准输入输出和 Headless 模式的入口点。
- xai-grok-tools：工具实现层，封装了终端操作、文件编辑、搜索等具体能力。
- xai-grok-workspace：宿主环境层，管理文件系统、版本控制系统（VCS）、执行环境和检查点。
- xai-grok-sandbox：沙箱模块，用于安全隔离代理的执行环境。
- xai-acp-lib：Agent Client Protocol 库，定义了代理与外部编辑器或客户端通信的协议。

#### 运行方式

- 安装预编译二进制文件：在 macOS/Linux 上使用 curl 脚本，在 Windows 上使用 PowerShell 脚本，安装后运行 grok --version 验证。
- 从源码构建：需要安装 Rust 工具链（由 rust-toolchain.toml 指定版本）和 DotSlash 工具（用于管理 hermetic 工具）。
- 构建命令：使用 cargo run -p xai-grok-pager-bin 启动 TUI，或使用 cargo build -p xai-grok-pager-bin --release 生成发布版二进制文件。
- 首次启动：程序会自动打开浏览器进行身份验证，需参考认证指南完成设置。
- 开发模式：推荐使用 cargo check -p <crate> 进行快速检查，避免全工作区构建。

#### 项目亮点

- 全屏交互式 TUI：提供类似 IDE 的沉浸式终端体验，支持滚动回显和模态交互。
- 多模式运行：支持交互式、无头（Headless）和嵌入式（ACP）三种运行模式，适应不同场景。
- 强大的代码库理解：内置代码库图和上下文感知能力，能够理解项目结构和依赖关系。
- 安全沙箱机制：通过沙箱技术限制代理在受控环境中执行命令，防止对宿主系统造成破坏。
- 丰富的工具生态：集成了文件编辑、终端操作、网络搜索等多种工具，并支持 MCP 服务器扩展。
- 高度模块化：代码结构清晰，分为 pager、shell、tools、workspace 等独立模块，便于维护和扩展。

#### 代码解析

- 项目根目录的 Cargo.toml 是自动生成的，实际开发应关注各个子 crate 的配置文件。
- 构建系统依赖 DotSlash 来管理 bin/ 目录下的工具（如 protoc），确保构建环境的可重复性。
- ptyctl-cli 是一个独立的命令行工具，用于在无头模式下控制 PTY 会话，提供了 run、wait、screen 等子命令。
- xai-acp-lib 定义了核心通信协议，包含消息格式、通道管理和标准化逻辑。
- xai-agent-lifecycle 模块管理代理的生命周期，包括会话状态、轮次输入和轮次生命周期。
- xai-grok-tools-api 提供了工具调用的接口定义，确保工具实现符合统一规范。
- 仓库中包含大量第三方库 vendoring（如 Mermaid 图表栈），位于 third_party/ 目录下。
...
- **语言**: C | **Star**: 20625 | **Fork**: 2090
- **更新**: 2026-07-29 | **License**: Apache-2.0

#### 前面介绍

- Colibrì 是一个纯 C 语言编写的轻量级混合专家模型（MoE）运行时引擎，旨在让用户在消费级硬件上运行超大规模模型。它通过将模型参数分层存储（RAM、磁盘、VRAM），并采用类似 JIT（即时编译）的按需加载机制，成功在约 25GB 内存的主机上运行了 744B 参数的 GLM-5.2 MoE 模型。该项目强调零依赖、质量无损以及完全的开源透明性，允许用户实时监控模型内部的专家路由行为。
...
- 核心引擎：单文件 C 实现，无 BLAS、无 Python 运行时依赖，无 GPU 强制要求。
- 内存分层：将 VRAM、RAM 和磁盘视为统一的管理层级，根据路由热度动态分配存储位置。
- 稀疏激活：744B 模型每 Token 仅激活约 40B 参数，其中约 11GB 为动态变化的专家参数。
- 按需加载：未激活的专家权重存储在磁盘，仅在路由器决定使用时从磁盘流式加载到内存。
- 缓存策略：采用层内 LRU 缓存、学习型固定热存储以及可选的 VRAM 专用层级。
- 路由机制：运行时预测下一层路由，利用预取隐藏加载延迟，类似编译器 JIT 优化热路径。
- 多后端支持：支持 CUDA、Metal 以及纯 CPU 路径，通过插件式架构实现。
- 专家分析：通过专门的探针工具测量专家路由热度和主题亲和度，生成可视化图谱。

#### 运行方式

- 环境准备：需要安装 C 编译器（如 GCC/Clang）和 Python 3 用于辅助工具。
- 依赖安装：通过 `pip install -r c/tools/requirements.txt` 安装 Python 依赖。
- 模型下载：使用 `c/tools/download_glm52.py` 下载 GLM-5.2 模型权重。
- 编译引擎：在 `c/` 目录下运行 Makefile 进行编译，生成 `coli` 可执行文件。
- 运行模型：使用 `./coli chat` 启动交互式对话，或 `./coli web` 启动可视化监控面板。
- 配置调整：通过环境变量或配置文件调整存储层级策略、缓存大小及路由参数。

#### 项目亮点

- 极致轻量化：核心引擎仅由一个 C 文件和少量头文件组成，代码量极小，易于理解和修改。
- 消费级运行：证明了在没有昂贵 GPU 的情况下，仅凭 25GB 内存和磁盘 IO 即可运行 744B 超大模型。
- 完全透明：不隐藏模型行为，默认策略绝不静默改变模型精度或路由语义。
- 可视化监控：提供 Web 仪表盘，实时展示 Token 生成速度、硬件负载以及 19,456 个专家的激活状态。
- 专家图谱：通过测量路由热度，构建了专家的 3D 星系图谱和主题亲和度分析，揭示了模型内部的稀疏结构。
- JIT 思想：借鉴编译器 JIT 技术，根据实际运行情况动态优化权重加载路径，提升推理效率。

#### 代码解析

- 核心逻辑位于 `c/colibri.c`，实现了 MoE 模型的前向传播和推理循环。
- `c/olmoe.c` 实现了稀疏 MoE 的核心路由算法，负责选择激活的专家。
- `c/backend_cuda.cu` 和 `c/backend_metal.mm` 分别封装了 CUDA 和 Metal 的 GPU 加速后端。
- `c/quant.h` 包含了量化算法，支持将模型权重从 FP8 转换为 INT4 以节省内存。
- `c/kv_persist.h` 处理键值对（KV Cache）的持久化存储，优化长上下文推理。
- `c/grammar.h` 和 `c/schema_gbnf.h` 实现了基于 GBNF 语法的生成控制。
- `c/telemetry.h` 收集运行时遥测数据，用于性能分析和专家图谱生成。
- `c/inkling.c` 可能包含用于模型推理的 Inkling 架构支持或特定优化逻辑。
...
##### c/tools/oracle-requirements.txt
...
- **语言**: JavaScript | **Star**: 12579 | **Fork**: 1255
- **更新**: 2026-07-29 | **License**: 未知
...
- **语言**: Python | **Star**: 10055 | **Fork**: 1317
- **更新**: 2026-07-29 | **License**: MIT
...
- **语言**: Rust | **Star**: 7720 | **Fork**: 13
- **更新**: 2026-07-29 | **License**: 未知

#### 前面介绍

- AOS Community Edition（AOS CE）是一个开源的智能体操作系统，旨在为智能体提供一个可检查、可组合的环境。它由 Unicity Labs 开发，主要使用 Rust 编写。AOS 提供了完整的社区版产品界面，包括 aos CLI、HTTP API、分发系统、第一方胶囊、模型体验以及 Unicity 审计功能。它将智能体视为操作系统中的用户空间构建块，通过胶囊（Capsules）机制实现模块化和可组合性。
...
- AOS 采用分层架构设计，将操作系统功能封装为独立的胶囊模块。
- 核心工作区包含 Crates（Rust 库）和 Capsules（用户空间构建块）。
- Crates 层负责产品 CLI、HTTP API、控制客户端和共享产品代码。
- Capsules 层包含 21 个第一方生产胶囊，提供具体的功能实现。
- CLI 作为显示服务器，通过 Unix Domain Socket 连接内核 IPC 事件总线与 TUI 前端。
- 系统支持多客户端并发连接，并维护严格的入站消息允许列表。
- 通过 Forge 工具，智能体可以构建和验证最小权限的胶囊。
- Meta Harness 技能教导智能体如何构建受治理的元 Harness。

#### 运行方式

- 访问官方安装脚本：curl --proto '=https' --tlsv1.2 -fsSL https://aos.unicity.ai/install.sh | sh
- 运行 aos init 初始化工作区，支持离线模式。
- 安装程序会安装 aos CLI、固定运行时以及 21 个社区版胶囊到 ~/.aos 目录。
- 使用 aos status 查看系统状态，支持 JSON 格式输出。
- 使用 aos daemon foreground 在 Unix 系统上启动持久化守护进程。
- 使用 aos mcp serve 启动模型上下文协议服务，支持多种交互模式。

#### 项目亮点

- 智能体操作系统模型：将智能体视为操作系统中的用户空间进程，而非单纯的脚本。
- 胶囊生态系统：提供 21 个第一方胶囊，支持最小权限原则和可组合性。
- Forge 构建工具：内置的操作系统构建工具，允许智能体检查、学习并构建新功能。
- Meta Harness 技能：内置技能，教导智能体如何构建受治理的元 Harness。
- 严格的命令边界：AOS 拥有产品级命令（如 init, status），运行时命令直接暴露，避免嵌套命名空间。
- MCP 协议集成：提供 aos mcp serve 命令，作为 Codex、Claude 等客户端的共享边缘接口。

#### 代码解析

- Workspace 配置：Cargo.toml 定义了包含 23 个成员的工作区，依赖 astrid-sdk 等核心库。
- 核心依赖：使用 tokio 作为异步运行时，axum 用于 HTTP API，serde 用于序列化。
- aos-cli 胶囊：作为显示服务器，绑定 Unix Socket，处理多客户端连接和 IPC 消息路由。
- 消息过滤：CLI 代理实现严格的入站消息允许列表，仅允许特定前缀或精确主题的消息通过。
- 胶囊-agents：负责将 AGENTS.md 文件中的项目指令注入到系统提示词中。
- 胶囊-forge：提供构建工具，包含指南文档，用于指导智能体构建胶囊。
- 构建优化：Release 配置启用了 LTO、代码单元优化和符号剥离，以减小二进制体积。
- 多语言支持：项目主要使用 Rust，但也包含少量 Python 和 Shell 脚本用于辅助工具。
...
- **语言**: Python | **Star**: 7662 | **Fork**: 588
- **更新**: 2026-07-29 | **License**: Apache-2.0
- **主题**: 3d、ai-agents、claude-code、computer-graphics、generative、image-to-3d、procedural-generation、threejs

#### 前面介绍

- img2threejs 是一个将参考图像中的物体重建为纯代码、程序化生成的 Three.js 模型的工具。它专注于生成动画就绪、质量受控且令牌高效的模型，而非依赖摄影测量、网格提取或下载的素材包。该项目通过 Python 脚本分析图像，提取几何特征和材质信息，最终输出可直接在浏览器中运行的 TypeScript/JavaScript 代码。
...
- 项目采用模块化设计，主要分为文档规范、核心脚本和演示展示三大部分。
- 文档规范层位于 `docs/specs/vocabulary`，定义了用于本地规范搜索的标准化 JSONL 记录结构，确保数据的一致性和可追溯性。
- 核心脚本层位于 `forge` 目录，包含多个阶段：`stage1_intake` 负责图像摄入和初步分析，`_shared` 模块提供共享工具如颜色度量、特征接受策略和规范搜索。
- 演示展示层位于 `img2threejs-showcase` 仓库，展示生成的模型并允许用户交互查看生成的源代码。
- 整个系统通过 Python 标准库实现，不依赖第三方重型库，保证了轻量化和高性能。

#### 运行方式

- 确保安装 Python 3.10 或更高版本。
- 克隆仓库并进入项目目录。
- 项目核心脚本位于 `forge` 文件夹，无需额外安装依赖，因为所有功能均使用 Python 标准库实现。
- 运行主脚本 `forge/next.py` 启动重建流程。
- 生成的代码将输出为 TypeScript/JavaScript，可直接集成到 Three.js 项目中。

#### 项目亮点

- 纯代码生成：生成的模型不是静态网格文件，而是可读、可编辑的代码，支持程序化修改。
- 令牌高效：设计上刻意优化了输入输出，降低了 Token 消耗，适合在 AI 代理或特定上下文中使用。
- 质量门控：包含严格的审查机制，确保生成的模型符合视觉质量和结构要求。
- 硬表面专家：特别擅长处理硬表面物体，如 CS2 武器、枪械和精密电子设备。
- 无外部依赖：核心脚本仅使用 Python 标准库，减少了环境配置的复杂度。

#### 代码解析

- 核心逻辑位于 `forge/next.py`，作为主入口点协调各个处理阶段。
- `forge/stage1_intake` 目录下的脚本负责图像的初步处理，包括纹理分析、细节属性绑定、CS2 物品检测以及 PBR（基于物理的渲染）证据提取。
- `forge/_shared/spec_search.py` 提供了 `load_jsonl_records` 函数，用于加载和验证符合规范记录格式的 JSONL 数据，确保数据完整性。
- `docs/specs/vocabulary/core_3d.jsonl` 和 `docs/specs/vocabulary/cs2.jsonl` 定义了重建过程中使用的核心 3D 词汇和特定领域（如 CS2）的解剖学规范。
- `forge/_shared/color_metrics.py` 和 `forge/_shared/feature_acceptance_policy.py` 等模块负责计算颜色差异和评估特征提取的准确性，作为质量门控的一部分。
...
| --- | --- | --- | --- |
| Glock-18 · Ghost Protocol (Well-Worn) | CS2 weapon | [Live](https://img2threejs.github.io/img2threejs-showcase/#/demo/glock-ghost-protocol) | [code](https://github.com/img2threejs/img2threejs-showcase/blob/main/src/
```
...
- **语言**: Python | **Star**: 6959 | **Fork**: 12742
- **更新**: 2026-07-29 | **License**: 未知
...
- **语言**: Rust
- **Star**: 23257 | **Fork**: 4404 | **更新**: 2026-07-29
...
- **语言**: C
- **Star**: 20625 | **Fork**: 2090 | **更新**: 2026-07-29

### 3. Fei-Away/Codex-Dream-Skin
- **仓库**: [Fei-Away/Codex-Dream-Skin](https://github.com/Fei-Away/Codex-Dream-Skin)
- **描述**: Codex Dream Skin
- **语言**: JavaScript
- **Star**: 12579 | **Fork**: 1255 | **更新**: 2026-07-29

### 4. andrewyng/openworker
- **仓库**: [andrewyng/openworker](https://github.com/andrewyng/openworker)
- **语言**: Python
- **Star**: 10053 | **Fork**: 1317 | **更新**: 2026-07-29

### 5. unicity-aos/aos-ce
- **仓库**: [unicity-aos/aos-ce](https://github.com/unicity-aos/aos-ce)
- **描述**: AOS Community Edition: the open agent operating system.
- **语言**: Rust
- **Star**: 7720 | **Fork**: 13 | **更新**: 2026-07-29
...
- **语言**: Python
- **Star**: 7660 | **Fork**: 588 | **更新**: 2026-07-29

### 7. x4gKing/X4G
- **仓库**: [x4gKing/X4G](https://github.com/x4gKing/X4G)
- **语言**: Python
- **Star**: 6959 | **Fork**: 12742 | **更新**: 2026-07-29
...
- **语言**: JavaScript
- **Star**: 5625 | **Fork**: 647 | **更新**: 2026-07-29
...
- **语言**: Python
- **Star**: 3255 | **Fork**: 255 | **更新**: 2026-07-29
